...
namespace System.Collections.ObjectModel
{
    ...
    public sealed class ObservableHashSet<T> : ISet<T>, INotifyCollectionChanged, INotifyPropertyChanged, IDisposable
    {
        ...
        public bool Add(T item)
        {
            ...
            if (wasAdded)
            {
                var index = -1;//this.hashSet.ToList().IndexOf(item);
        ...
        private void IsNotNull(string p, IEnumerable<T> other)
        {
            if (other == null)
            {
                throw new InvalidOperationException("Other set is null");
            }
        }
        ...
        {
            var index = -1; //this.hashSet.IndexOf(item);
