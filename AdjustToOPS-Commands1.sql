...
WHERE (AdjustmentDetails.EffectiveDate BETWEEN @startdate AND @endDate) and (ISNULL(AdjustmentDetails.ReceivedQty, 0) < ISNULL(AdjustmentDetails.InvoiceQty, 0))
...
GROUP BY CASE WHEN AliasName IS NOT NULL THEN inventoryitemalias.itemnumber ELSE AdjustmentDetails.ItemNumber END, AdjustmentDetails.ItemDescription, AdjustmentDetails.EntryDataId, 
                 AdjustmentDetails.EffectiveDate,  AdjustmentDetails.IsReconciled, AdjustmentDetails.EntryDataDetailsId
...
SELECT        ItemNumber,max(ItemDescription) as ItemDescription, abs(SUM(Quantity)) AS Quantity, 
...
FROM     [#P2O-Overs] as res inner join  EntryDataDetails  on res.ItemNumber = Entrydatadetails.ItemNumber INNER JOIN
...
FROM     [#P2O-Shorts] as res inner join  EntryDataDetails  on res.ItemNumber = Entrydatadetails.ItemNumber INNER JOIN
                         AsycudaSalesAllocations ON EntryDataDetails.EntryDataDetailsId = AsycudaSalesAllocations.EntryDataDetailsId INNER JOIN
                         EntryData_Sales ON EntryDataDetails.EntryDataId = EntryData_Sales.EntryDataId INNER JOIN
                         EntryData ON EntryData_Sales.EntryDataId = EntryData.EntryDataId
...
FROM     [#A2O-Overs] as res inner join  EntryDataDetails  on res.ItemNumber = Entrydatadetails.ItemNumber INNER JOIN
                         AsycudaSalesAllocations ON EntryDataDetails.EntryDataDetailsId = AsycudaSalesAllocations.EntryDataDetailsId INNER JOIN
                         EntryData_Sales ON EntryDataDetails.EntryDataId = EntryData_Sales.EntryDataId INNER JOIN
                         EntryData ON EntryData_Sales.EntryDataId = EntryData.EntryDataId                         
WHERE        (EntryData.EntryDataDate BETWEEN @startDate AND @enddate) and AsycudaSalesAllocations.Status is not null
...
FROM     [#A2O-Shorts] as res inner join  EntryDataDetails  on res.ItemNumber = Entrydatadetails.ItemNumber INNER JOIN
                         AsycudaSalesAllocations ON EntryDataDetails.EntryDataDetailsId = AsycudaSalesAllocations.EntryDataDetailsId INNER JOIN
                         EntryData_Sales ON EntryDataDetails.EntryDataId = EntryData_Sales.EntryDataId INNER JOIN
                         EntryData ON EntryData_Sales.EntryDataId = EntryData.EntryDataId                         
WHERE        (EntryData.EntryDataDate BETWEEN @startDate AND @enddate) and AsycudaSalesAllocations.Status is not null
...
--TODO: check out AOR/151550F, TAY/71026
							...
							---  XAN/813-0400-01 +1
							---- vs SPY/0799320 0
declare @ItemNumber varchar(50) = 'FAA/SRTH34X10'
select 'Unexecuted Adjustments'
...
FROM   AsycudaSalesAllocationsEx as AdjustmentShortAllocations LEFT OUTER JOIN
                 AsycudaItemPiQuantityData ON AdjustmentShortAllocations.PreviousItem_Id = AsycudaItemPiQuantityData.Item_Id
WHERE ((AsycudaItemPiQuantityData.Item_Id IS NULL)) 