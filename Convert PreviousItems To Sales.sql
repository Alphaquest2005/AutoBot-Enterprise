SELECT 'Asycuda-C#' + PreviousItemsEx.CNumber AS 'Invoice #', AsycudaDocument.RegistrationDate AS Date, PreviousItemsEx.Prev_decl_HS_spec AS ItemNumber, InventoryItems.Description, 
...
WHERE (PreviousItemsEx.ApplicationSettingsId = 7) AND (AsycudaDocument.RegistrationDate <= '10/1/2021')
...
WHERE (EntryData.ApplicationSettingsId = 7) AND (EntryDataDetails.EntryDataId LIKE N'Asycuda-C#%')
...
WHERE (InventoryItems.ApplicationSettingsId = 7) AND (InventorySources.Name = N'Asycuda')
