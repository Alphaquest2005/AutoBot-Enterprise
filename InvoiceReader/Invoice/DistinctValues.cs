...
public partial class Invoice
{
    ...
    {
        int? invoiceId = this.OcrInvoices?.Id; // For logging context
        _logger.Debug("Entering DistinctValues for InvoiceId: {InvoiceId}. Input dictionary count: {InputCount}", invoiceId, lineValues?.Count ?? 0);

        // Null check for input
        if (lineValues == null)
        {
            _logger.Warning("DistinctValues called with null input dictionary for InvoiceId: {InvoiceId}. Returning new empty dictionary.", invoiceId);
            return new Dictionary<(int lineNumber, string section), Dictionary<(Fields fields, int instance), string>>();
        }
        ...
        try
        {
            foreach (var val in lineValues)
            {
                ...
                var currentValueDict = val.Value; // The inner dictionary
                 _logger.Verbose("Processing item {ProcessedCount}/{TotalCount}: Key=({LineNumber}, {Section}) for InvoiceId: {InvoiceId}",
                 ...
                 {
                      _logger.Verbose("Skipping item {ProcessedCount} because its inner dictionary (Value) is null or empty. Key: {Key}", processedCount, currentKey);
                      continue;
                 }
                 ...
                 if (!alreadyExists)
                 {
                      _logger.Verbose("Adding item with Key: {Key} because its values [{Values}] are distinct from previously added items.",
                 ...
                 {
                      _logger.Verbose("Skipping item with Key: {Key} because at least one of its values [{Values}] already exists in the results.",
                         currentKey, string.Join(",", currentValueDict.Values));
                 }
            }

             _logger.Debug("Finished DistinctValues for InvoiceId: {InvoiceId}. Processed: {ProcessedCount}, Added: {AddedCount}, Result Count: {ResultCount}",
