    ...
    private void AddMissingRequiredFieldValues()
    {
        ...
        try
        {
            ...
            var requiredFieldsList = Lines
                ...
                .Where(z => z != null && z.IsRequired && z.FieldValue != null && !string.IsNullOrEmpty(z.FieldValue.Value)) // Safe checks
                .ToList();
             _logger.Debug("Found {Count} required fields with values for InvoiceId: {InvoiceId}", requiredFieldsList.Count, invoiceId);
             if (!requiredFieldsList.Any())
             {
                 _logger.Debug("No required fields with values found, exiting AddMissingRequiredFieldValues for InvoiceId: {InvoiceId}", invoiceId);
            ...
            var instances = Lines
                ...
                .ToList();
             _logger.Debug("Found {Count} distinct instances from 'Single' sections for InvoiceId: {InvoiceId}", instances.Count, invoiceId);
             if (!instances.Any())
             {
                  _logger.Debug("No instances found in 'Single' sections, exiting AddMissingRequiredFieldValues for InvoiceId: {InvoiceId}", invoiceId);
                  return; // Nothing to do
             }
            ...
            foreach (var field in requiredFieldsList)
            {
                 ...
                 _logger.Verbose("Processing Required FieldId: {FieldId} ('{FieldName}') for InvoiceId: {InvoiceId}", fieldId, field?.Field, invoiceId);
                 ...
                 var lines = Lines.Where(x => x != null && // Ensure line is not null
                                              ...
                                              ValueForAllInstances(x, instances) // Assuming this checks if the value exists for *all* specified instances
                 ).ToList();
                  _logger.Verbose("Found {Count} lines matching criteria for FieldId: {FieldId}", lines.Count, fieldId);
                 ...
                 foreach (var line in lines)
                 {
                     ...
                     var lineInstances = line?.Values?.SelectMany(z => z.Value?.Keys.Select(k => k.instance) ?? Enumerable.Empty<int>()).Distinct().ToList()
                                        ?? new List<int>(); // Safe navigation and default
                      _logger.Verbose("LineId: {LineId} already has values for instances: [{Instances}]", lineId, string.Join(",", lineInstances));
                     ...
                     foreach (var instance in instances)
                     {
                         ...
                         {
                              _logger.Debug("Adding missing value for FieldId: {FieldId}, Instance: {Instance}, LineNumber: {LineNumber} to LineId: {LineId}",
                             ...
                             var valueToAdd = field.FieldValue.Value; // Already checked FieldValue/Value is not null/empty

                             // Safely add to the dictionary
                             // Check if the main Values dictionary exists FIRST
                             if (line.Values == null)
                             {
                                 _logger.Warning("Cannot add missing value for FieldId: {FieldId}, Instance: {Instance} because Line.Values dictionary is null for LineId: {LineId}. Skipping.",
                             ...
                             if (line.Values.TryGetValue(key, out var innerDict))
                             {
                                 ...
                                 {
                                      _logger.Warning("Inner dictionary for Key ({LineNumber}, {Section}) is null for LineId: {LineId}. Initializing.", instance.LineNumber, "Single", lineId);
                                 ...
                                 if (!innerDict.ContainsKey(innerValueKey))
                                 {
                                     innerDict.Add(innerValueKey, valueToAdd);
                                      _logger.Verbose("Added value to existing inner dictionary for LineId: {LineId}, Key: {Key}, InnerKey: (FieldId:{FieldId}, Instance:{Instance})",
                                         lineId, key, fieldId, instance.Instance);
                                 }
                                 else
                                 {
                                      _logger.Warning("Attempted to add value for FieldId: {FieldId}, Instance: {Instance} to LineId: {LineId}, but it already exists. Ignoring.", fieldId, instance.Instance, lineId);
                                 }
                             }
                             else
                             {
                                 // Key doesn't exist, add new entry with the inner dictionary
                                  _logger.Verbose("Adding new key and inner dictionary for LineId: {LineId}, Key: {Key}, InnerKey: (FieldId:{FieldId}, Instance:{Instance})",
                                     lineId, key, fieldId, instance.Instance);
                                 line.Values.Add(key,
                              ...
                              _logger.Verbose("LineId: {LineId} already contains instance {Instance} for FieldId: {FieldId}. Skipping.", lineId, instance.Instance, fieldId);
                         }
                     }
                 }
            }
