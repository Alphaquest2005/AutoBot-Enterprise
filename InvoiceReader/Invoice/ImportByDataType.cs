...
public partial class Invoice
{
        ...
        KeyValuePair<(int lineNumber, string section), Dictionary<(Fields fields, int instance), string>> value)
    {
        ...
        _logger.Verbose("Entering ImportByDataType for FieldId: {FieldId} ('{FieldName}'), Instance: {Instance}, DataType: {DataType}, Append: {Append}, LineKey: {LineKey}",
        ...
        if (ditm == null)
        {
             _logger.Error("ImportByDataType cannot proceed: target dictionary 'ditm' is null. FieldId: {FieldId}", fieldId);
             return;
        }
         // value (KeyValuePair) itself is unlikely null if called from loop, but inner dictionary could be
         if (value.Value == null)
         {
              _logger.Warning("ImportByDataType called with null inner dictionary in 'value' parameter for FieldId: {FieldId}, LineKey: {LineKey}. GetValueByKey will likely return null.", fieldId, lineKey);
         }
        ...
        try
        {
            ...
            {
                 _logger.Debug("Field '{FieldName}' not found in target dictionary 'ditm'. Initializing based on DataType: {DataType}.", fieldName, dataType);
            ...
            _logger.Verbose("Calling GetValueByKey for FieldKey: {FieldKey}, LineKey: {LineKey}", fieldKey, lineKey);
            dynamic retrievedValue = GetValueByKey(value, fieldKey); // GetValueByKey logs its own details
             _logger.Verbose("GetValueByKey returned: {Value} (Type: {Type}) for FieldKey: {FieldKey}", retrievedValue, retrievedValue?.GetType().Name ?? "null", fieldKey);
            ...
            switch (dataType)
            {
                case "String":
                     ...
                     ditm[fieldName] = string.IsNullOrEmpty(existingString) ? newString : (existingString + " " + newString).Trim();
                ...
                case "Numeric":
                     ...
                     try
                     {
                         if (ditm[fieldName] is double currentDbl) {
                             ...
                             currentParsed = double.TryParse(ditm[fieldName].ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out currentValue);
                         } else {
                             currentParsed = true; // Treat null as 0.0
                         }
                     }
                     catch (Exception convEx) { _logger.Warning(convEx, "Could not convert existing ditm value '{ExistingValue}' to double for Field '{FieldName}'. Using 0.", ditm[fieldName], fieldName); }
                     ...
                     try
                     {
                         if (retrievedValue is double newDbl) {
                             ...
                             newParsed = double.TryParse(retrievedValue.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out newValue);
                         } else {
                             newParsed = true; // Treat null as 0.0
                         }
                     }
                     catch (Exception convEx) { _logger.Warning(convEx, "Could not convert retrieved value '{RetrievedValue}' to double for Field '{FieldName}'. Using 0.", retrievedValue, fieldName); }
                     ...
                     if (appendValues)
                     {
                         ...
                         _logger.Verbose("AppendValues is true. Adding new value {NewValue} to existing {CurrentValue} for Field '{FieldName}'.", newValue, currentValue, fieldName);
                         ditm[fieldName] = currentValue + newValue;
                     }
                     else // appendValues is false
                     {
                         // Corrected logic: Replace the value if append is false.
                         _logger.Verbose("AppendValues is false. Replacing existing value {CurrentValue} with new value {NewValue} for Field '{FieldName}'.", currentValue, newValue, fieldName);
                ...
                default: // Includes Date, English Date, and any unknown types
                     ...
                     ditm[fieldName] = retrievedValue;
                      _logger.Verbose("Updated ditm['{FieldName}'] to: {Value} (Type: {Type})", fieldName, ditm[fieldName], ditm[fieldName]?.GetType().Name ?? "null");
                    break;
            }
        ...
        catch (Exception e)
        {
             ...
             _logger.Error(e, "Error during ImportByDataType for FieldId: {FieldId} ('{FieldName}'), Instance: {Instance}, LineKey: {LineKey}",
