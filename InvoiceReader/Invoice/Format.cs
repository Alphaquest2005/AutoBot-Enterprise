...
public partial class Invoice
{
    ...
    public string Format(string pdftxt)
    {
        ...
        _logger.Debug("Entering Format method for InvoiceId: {InvoiceId}. Input text length: {Length}", invoiceId, pdftxt?.Length ?? 0);
        ...
        {
            _logger.Warning("Cannot format text: OcrInvoices or OcrInvoices.RegEx is null for InvoiceId: {InvoiceId}. Returning original text.", invoiceId);
            return pdftxt ?? string.Empty; // Return original or empty string if null
        }
         if (pdftxt == null)
         {
              _logger.Warning("Cannot format text: Input pdftxt is null for InvoiceId: {InvoiceId}. Returning empty string.", invoiceId);
              return string.Empty;
         }
        ...
        try
        {
            ...
            var regexList = regexCollection.OrderBy(x => x?.Id ?? int.MaxValue).ToList(); // Handle potential nulls in list during sort
            _logger.Debug("Applying {Count} formatting regex patterns sequentially for InvoiceId: {InvoiceId}.", regexList.Count, invoiceId);
            int appliedCount = 0;
            ...
            foreach (var reg in regexList)
            {
                ...
                {
                     _logger.Warning("Skipping formatting step: RegEx info or pattern is null/empty for RegId: {RegId}, InvoiceId: {InvoiceId}", regId, invoiceId);
                     continue; // Skip this iteration
                }
                 if (reg.ReplacementRegEx == null)
                 {
                      // Log if the replacement object itself is null, leading to empty string replacement
                      _logger.Warning("ReplacementRegEx object is null for RegId: {RegId}, InvoiceId: {InvoiceId}. Replacement will be empty string.", regId, invoiceId);
                 }

                 _logger.Verbose("Applying RegId: {RegId} - Pattern: '{Pattern}', Replacement: '{Replacement}' for InvoiceId: {InvoiceId}",
                 ...
                 try
                 {
                     ...
                     pdftxt = Regex.Replace(pdftxt, pattern, replacement,
                         RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.ExplicitCapture, RegexTimeout);
                     appliedCount++;
                      _logger.Verbose("Text after applying RegId: {RegId}. New Length: {Length}", regId, pdftxt.Length);
                 }
                 catch (RegexMatchTimeoutException timeoutEx)
                 {
                      _logger.Error(timeoutEx, "Regex replace timed out (>{TimeoutSeconds}s) for RegId: {RegId}, Pattern: '{Pattern}', InvoiceId: {InvoiceId}. Skipping this replacement.",
                 ...
                 {
                      _logger.Error(argEx, "Invalid regex pattern encountered for RegId: {RegId}, Pattern: '{Pattern}', InvoiceId: {InvoiceId}. Skipping this replacement.",
                         regId, pattern, invoiceId);
                 }
                 catch (Exception regexEx) // Catch other regex errors
                 {
                      _logger.Error(regexEx, "Error applying regex replace for RegId: {RegId}, Pattern: '{Pattern}', InvoiceId: {InvoiceId}. Skipping this replacement.",
                         regId, pattern, invoiceId);
                      // Continue with the next regex
                 }
            }

             _logger.Debug("Finished formatting text for InvoiceId: {InvoiceId}. Applied {AppliedCount}/{TotalCount} patterns. Final text length: {Length}",
