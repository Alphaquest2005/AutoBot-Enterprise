            ...
            try
            {
                ...
                if (f.Key.fields == null)
                {
                    _logger.Warning(
                        ...
                        methodName, fieldId);
                    _logger.Verbose("Exiting {MethodName} for FieldId: {FieldId} (Null Fields). Returning null.",
                        methodName, fieldId);
                    return null;
                }
                ...
                // --- Type Conversion ---
                _logger.Verbose("{MethodName}: Performing conversion based on DataType: {DataType}", methodName,
                    dataType);
                switch (dataType)
                {
                    case "String":
                    ...
                    case "Number":
                        ...
                        else
                        {
                            ...
                            _logger.Verbose("{MethodName}: Cleaned numeric value: '{CleanedValue}' (from '{RawValue}')",
                            ...
                            if (double.TryParse(cleanedValue, NumberStyles.Any, CultureInfo.InvariantCulture,
                                    out double num))
                            {
                                _logger.Verbose("{MethodName}: Successfully parsed as double: {Value}", methodName,
                                    num);
                                result = num;
                            }
                            else
                    ...
                    case "Date":
                        ...
                        else
                        {
                            ...
                            if (DateTime.TryParse(trimmedDate, CultureInfo.InvariantCulture, DateTimeStyles.None,
                                    out DateTime date))
                            {
                                _logger.Verbose("{MethodName}: Successfully parsed as DateTime: {Value}", methodName,
                                    date);
                                result = date;
                            }
                            else
                    ...
                    case "English Date":
                        ...
                        foreach (String formatString in formatStrings)
                        {
                            ...
                            {
                                _logger.Verbose("{MethodName}: Failed to parse using format '{Format}'.", methodName,
                                    formatString);
                            }
                        }
