    ...
    public partial class Part
    {
        public List<Line> AllLines
        {
            get
            {
                ...
                {
                    // --- Get Direct Lines ---
                    _logger.Verbose("{PropertyName}: Getting direct lines for PartId: {PartId}...", propertyName,
                        partId);
                    ...
                    // --- Get Child Lines ---
                    _logger.Verbose("{PropertyName}: Getting child lines for PartId: {PartId}...", propertyName,
                        partId);
                    var childLines = this.ChildParts?
                        .Where(cp => cp != null) // Safe check
                        .SelectMany(x =>
                            x.AllLines ?? Enumerable.Empty<Line>()) // Access property recursively, handle null
                        .ToList() ?? new List<Line>();
                    _logger.Verbose("{PropertyName}: Found {Count} lines from children for PartId: {PartId}.",
                        propertyName,
