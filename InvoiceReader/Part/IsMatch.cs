...
public partial class Part
{
    ...
    private static Match IsMatch(string val, Start z) // Kept static IsMatch for potential reuse?
    {
        ...
        _logger.Verbose("Entering static IsMatch for PartId: {PartId}, Pattern: '{Pattern}'. Input string length: {Length}",
            partId, pattern, val?.Length ?? 0);

        // Null checks for critical inputs
        if (z?.RegularExpressions == null || string.IsNullOrEmpty(pattern))
        {
             _logger.Warning("Cannot perform IsMatch: Start condition or Regex pattern is null/empty for PartId: {PartId}", partId);
             return System.Text.RegularExpressions.Match.Empty; // Return non-null empty match
        }
         if (val == null)
         {
              _logger.Warning("Cannot perform IsMatch: Input string 'val' is null for PartId: {PartId}, Pattern: '{Pattern}'", partId, pattern);
              return System.Text.RegularExpressions.Match.Empty; // Return non-null empty match
         }
        ...
        try
        {
            ...
            var options = (z.RegularExpressions.MultiLine == true ? RegexOptions.Multiline : RegexOptions.Singleline)
             ...
             _logger.Verbose("Performing Regex.Match for PartId: {PartId}, Pattern: '{Pattern}', Options: {Options}", partId, pattern, options);
        ...
        {
             _logger.Error(timeoutEx, "Regex Timeout (>{Timeout}s) in static IsMatch: PartId={PartId}, Pattern='{Pattern}'",
        ...
        {
             _logger.Error(argEx, "Invalid Regex Pattern in static IsMatch: PartId={PartId}, Pattern='{Pattern}'", partId, pattern);
             return System.Text.RegularExpressions.Match.Empty; // Return non-null empty match on invalid pattern
        }
        catch (Exception e)
        {
             _logger.Error(e, "Unexpected error in static IsMatch: PartId={PartId}, Pattern='{Pattern}'", partId, pattern);
