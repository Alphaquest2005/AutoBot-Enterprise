...
namespace WaterNut.DataSpace
{
    public partial class Line
    {
            ...
            Dictionary<(Fields Fields, int Instance), string> values)
        {
            ...
            _logger.Verbose(
                "Entering {MethodName} for LineId: {LineId}, Instance: {Instance}. Input Match Count: {MatchCount}",
                methodName, lineId, instance, matches?.Count ?? 0);

            // --- Input Validation ---
            if (matches == null)
            {
                _logger.Warning("{MethodName}: Called with null matches collection for LineId: {LineId}. Exiting.",
                    methodName, lineId);
            ...
            foreach (Match match in matches)
            {
                matchIndex++;
                _logger.Verbose(
                    "{MethodName}: Processing Match {MatchIndex}/{TotalMatches} for LineId: {LineId}, Instance: {Instance}",
                    methodName, matchIndex + 1, matches.Count, lineId, instance);
                if (match == null || !match.Success)
                {
                    _logger.Warning(
                        "{MethodName}: Skipping null or unsuccessful match at index {MatchIndex} for LineId: {LineId}",
                        methodName, matchIndex, lineId);
                    continue;
                }
                ...
                _logger.Verbose("{MethodName}: Match {MatchIndex}: Processing {FieldCount} top-level fields.",
                    methodName, matchIndex + 1, topLevelFields.Count);
                ...
                foreach (var field in topLevelFields)
                {
                    ...
                    _logger.Verbose(
                        "{MethodName}: Match {MatchIndex}: Processing Field {FieldIndex}/{TotalFields} (Id: {FieldId}, Name: '{FieldName}') for Instance: {Instance}",
                        ...
                        formattedValue != null) // Only format if value exists and formats defined
                    {
                        ...
                        _logger.Verbose(
                            "{MethodName}: FieldId: {FieldId} - Finished applying format regex patterns. Final formatted value: '{FinalValue}'",
                            methodName, fieldId, formattedValue);
                    }
