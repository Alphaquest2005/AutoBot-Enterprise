...
namespace WaterNut.DataSpace
{
    public partial class OCRCorrectionService
    {
        ...
        /// <summary>
        /// ENHANCED INVOICE ANALYSIS (V14.0): Creates a prompt with mandatory field completion requirements for complete data extraction.
        /// </summary>
        ...
        {
            _logger.Information("🚀 **PROMPT_CREATION_START (V14.0 - Enhanced Mandatory Completion)**: Creating prompt with mandatory field requirements for invoice {InvoiceNo}", invoice?.InvoiceNo ?? "NULL");
...
Your primary goal is to find all missing values in the text that account for this discrepancy.";
            ...
            var prompt = $@"ENHANCED INVOICE ANALYSIS (V14.0 - Complete Data Extraction):

**CONTEXT:**
You are analyzing an invoice to identify missing or incorrect fields. You MUST locate actual text in the OCR data and provide complete extraction details for every error found.
...
1. **field**: The exact field name (e.g., ""TotalOtherCost"", ""InvoiceNo"")
2. **correct_value**: The actual value from the OCR text (e.g., ""8.40"", ""111-8019845-2302666"")
3. **error_type**: ""omission"" or ""format_correction"" or ""multi_field_omission""
4. **line_number**: The actual line number where the value appears (1-based indexing)
5. **line_text**: The complete text of that line from the OCR
6. **suggested_regex**: A working regex pattern that captures the value with named groups
7. **reasoning**: Explain why this value was missed and how to extract it

**NEVER RETURN NULL VALUES** for line_number, line_text, or suggested_regex unless the field truly cannot be found.

**🔍 TEXT ANALYSIS PROCEDURE:**

**Step 1: LOCATE THE VALUE**
- Search through the OCR text line by line
- Find the exact line containing the missing value
- Note the line number (count from line 1)
- Record the complete line text

**Step 2: CREATE EXTRACTION PATTERN**
- Write a regex that captures the value with a named group
- Test mentally: does this pattern match the line text?
- Ensure the pattern is specific enough to avoid false matches
- Use named groups: (?<FieldName>pattern)

**Step 3: VALIDATE COMPLETENESS**
- Verify line_number corresponds to actual OCR line
- Verify line_text contains the correct_value
- Verify suggested_regex would extract correct_value from line_text
...
```json
{
  ...
  ""confidence"": 0.9,
  ""error_type"": ""omission"",
  ""reasoning"": ""Found 'Estimated tax: $8.40' on line 15 which represents TotalOtherCost but was not extracted by the system"",
  ""line_number"": 15,
  ""line_text"": ""Estimated tax to be collected: $8.40"",
  ""context_lines_before"": [""Shipping & Handling: $43.00""],
  ""context_lines_after"": [""Grand Total: $171.37""],
  ""requires_multiline_regex"": false,
  ""suggested_regex"": ""Estimated tax.*?\\$(?<TotalOtherCost>[\\d,]+\\.\\d{2})"",
  ""captured_fields"": [""TotalOtherCost""],
  ""field_corrections"": [],
  ""pattern"": null,
  ""replacement"": null
}
...
**🏗️ OBJECT-ORIENTED ENHANCEMENTS:**

**MULTI-FIELD OBJECTS** (when multiple fields appear together):
- Use error_type: ""multi_field_omission""
- Create suggested_regex with multiple named groups
- List all captured_fields: [""Quantity"", ""ItemDescription"", ""UnitPrice""]
- Example: ""3 of: Product Name $39.99"" → (?<Quantity>\\d+)\\s*of:\\s*(?<ItemDescription>.+?)\\$(?<UnitPrice>[\\d,]+\\.\\d{2})

**FORMAT CORRECTIONS** (when value exists but needs cleaning):
- Set pattern: the text that needs fixing
- Set replacement: the corrected text
- Example: pattern: ""BOTTOMK NT"", replacement: ""BOTTOM PAINT""

**Caribbean Customs Field Mapping:**
- **SUPPLIER REDUCTIONS** (Free Shipping, Discounts) → field: ""TotalDeduction"", positive values
- **CUSTOMER REDUCTIONS** (Gift Cards, Credits) → field: ""TotalInsurance"", negative values

**YOUR TASK:**
Analyze the OCR text systematically. For every missing field in the extracted data, locate it in the OCR text and provide a COMPLETE error object with all required fields filled.

**RETURN FORMAT:**
```json
{
  ""errors"": [
    // Array of complete error objects - NO NULL VALUES for critical fields
  ]
}
```

Analyze now and provide complete extraction details for every error found:";

            _logger.Information("🏁 **PROMPT_CREATION_COMPLETE (V14.0)**: Enhanced invoice analysis prompt with mandatory field completion requirements created. Length: {PromptLength} characters.", prompt.Length);
            return prompt;
        }
...
- Focus on fixing the specific reason for failure.";
        ...
        private string CreateProductErrorDetectionPrompt(ShipmentInvoice invoice, string fileText)
        {
            ...
            var prompt = "OBJECT-ORIENTED INVOICEDETAIL ENTITY DETECTION (V14.0):" + Environment.NewLine + Environment.NewLine +
                ...
                "   For ItemDescription spanning multiple lines, use NUMBERED CAPTURE GROUPS:" + Environment.NewLine +
                ...
                "   - Production will create ItemDescription2, ItemDescription3 fields with Append=true" + Environment.NewLine + Environment.NewLine +
                ...
                "**✅ CORRECT SINGLE-REGEX APPROACH**:" + Environment.NewLine +
                $"Pattern: \"{productDocMultiLineRegex}\"" + Environment.NewLine +
                "RequiresMultilineRegex: true" + Environment.NewLine +
                $"Applied to: \"3 of: PRODUCT NAME{EscapeRegexForDocumentation(@"\n")}CONTINUATION $39.99\"" + Environment.NewLine +
                $"Result: With Singleline mode, .+? captures \"PRODUCT NAME{EscapeRegexForDocumentation(@"\n")}CONTINUATION\" completely" + Environment.NewLine + Environment.NewLine +
                "**✅ CORRECT NUMBERED-GROUPS APPROACH (Alternative)**:" + Environment.NewLine +
                ...
                "captured_fields: [\"Quantity\", \"ItemDescription\", \"ItemDescription2\", \"UnitPrice\"]" + Environment.NewLine + Environment.NewLine +
                ...
                "2. Choose single .+? OR numbered groups - both work but serve different needs" + Environment.NewLine +
                "3. Test your pattern mentally before submitting - if it can't extract CorrectValue, fix it!" + Environment.NewLine +
                "4. Include ALL numbered groups in captured_fields array" + Environment.NewLine + Environment.NewLine +
                ...
                "- suggested_regex: REQUIRED - C# regex pattern with named capture groups" + Environment.NewLine +
...
//         private string CreateHeaderErrorDetectionPrompt(ShipmentInvoice invoice, string fileText, Dictionary<string, OCRFieldMetadata> metadata)
//         {
//             _logger.Information("🚀 **PROMPT_CREATION_START (V13.0 - Object-Oriented Architecture)**: Creating object-aware prompt for invoice {InvoiceNo}", invoice?.InvoiceNo ?? "NULL");
...
//             var prompt = $@"OBJECT-ORIENTED INVOICE ANALYSIS (V13.0 - Business Entity Framework):
...
// **3. COMPLETE OCR TEXT:**
// {this.CleanTextForAnalysis(fileText)}{Environment.NewLine}
...
// - **Entity Type**: Header fields must have entity_type = ""ShipmentInvoice""
// - **Regex Compatibility**: suggested_regex must use C# compatible patterns with named capture groups  
// - **Field Mapping**: Field names must exactly match database field names (InvoiceTotal, SupplierName, Currency, etc.)
...
// **CRITICAL: OCR CHARACTER CONFUSION IN HEADERS**:
// For header fields with OCR character confusion (e.g., ""0"" vs ""O"", ""1"" vs ""l""):

// **WRONG APPROACH - DO NOT DO THIS**:
// {{
//   ""suggested_regex"": ""Invoice Total:\\s*\\$(?<InvoiceTotal>[\\d\\.]+)"",
//   ""error_type"": ""omission""
// }}
// ❌ This tries to match clean text when the source contains OCR errors!

// **CORRECT APPROACH - USE FORMAT CORRECTION**:
// {{
//   ""field"": ""InvoiceNo"",
//   ""extracted_value"": ""l23456"",
//   ""correct_value"": ""123456"", 
//   ""error_type"": ""format_correction"",
//   ""suggested_regex"": ""Invoice No:\\s*(?<InvoiceNo>[l\\d]+)"",
//   ""pattern"": ""^l"",
//   ""replacement"": ""1"",
//   ""reasoning"": ""OCR confusion: leading 'l' should be '1'""
// }}
// ✅ This captures the actual OCR text, then corrects it!

// **🚨 MANDATORY REGEX VALIDATION AND MULTI-LINE REQUIREMENTS FOR HEADERS**:
// Before submitting your response, you MUST:

// **1. MULTI-LINE DETECTION**:
//    - Check if line_text contains \\n (newlines)
//    - If YES: Set "RequiresMultilineRegex": true (enables RegexOptions.Singleline in production)
//    - If NO: Set "RequiresMultilineRegex": false (uses default regex mode)

// **2. VALIDATION PROCESS**:
// 1. **Take your suggested_regex pattern**
// 2. **Apply it to the line_text you provided**
// 3. **Verify it matches and captures the intended value**
// 4. **Set RequiresMultilineRegex: true if line_text contains \\n**
// 5. **If it doesn't match, fix the regex before submitting**

// **EXAMPLE VALIDATION**:
// ```
// Line Text: ""Invoice Total: $l,49O.9O""
// Regex: ""Invoice Total:\\s*\\$(?<InvoiceTotal>[l\\d,O\\.]+)""

// ✅ VALIDATION CHECK: This regex WILL match ""l,49O.9O"" (including OCR errors)
// ❌ BAD EXAMPLE: ""Invoice Total:\\s*\\$(?<InvoiceTotal>[\\d,\\.]+)"" would NOT match because it excludes 'l' and 'O' characters
// ```

// **MULTI-LINE HEADER VALIDATION**:
// ```
// Line Text: ""Supplier: ACME Corp\\nAddress: 123 Main St""
// Regex: ""Supplier:\\s*(?<SupplierName>.+?)\\s*Address:""

// ✅ VALIDATION CHECK: With Singleline mode, .+? captures across newlines
// ❌ BAD EXAMPLE: Without considering newlines, pattern would fail
// ```

// **CRITICAL**: If your regex cannot extract the correct_value from the provided line_text, your response is WRONG and will fail in production!
...
// For header fields that span multiple lines (e.g., multi-line supplier addresses):
// - Set ""requires_multiline_regex"": true
// - Include ""context_lines_before"" and ""context_lines_after"" arrays
// - Use \\\\r\\\\n for line breaks in regex patterns
// - Example for multi-line supplier address:
// ```json
// {{
//   ""field"": ""SupplierAddress"",
//   ""extracted_value"": ""null"",
//   ""correct_value"": ""123 Main St\\nAnytown, ST 12345\\nUnited States"",
//   ""line_text"": ""Budget Corporation"",
//   ""line_number"": 15,
//   ""confidence"": 0.95,
//   ""error_type"": ""omission"",
//   ""entity_type"": ""ShipmentInvoice"",
//   ""suggested_regex"": ""^Budget.*\\\\r\\\\n\\\\r\\\\n(?<SupplierAddress>[A-Z\\\\s\\\\d\\\\r\\\\n\\\\.\\\\,:()]*)"",
//   ""requires_multiline_regex"": true,
//   ""context_lines_before"": [""Supplier Information:""],
//   ""context_lines_after"": [""Phone: 555-123-4567"", ""Email: info@budget.com""],
//   ""reasoning"": ""Supplier address spans multiple lines below the supplier name.""
// }}
// ```

// **🚨 FINAL VALIDATION CHECKLIST - MANDATORY COMPLIANCE FOR HEADERS**:

// **BEFORE SUBMITTING YOUR RESPONSE, VALIDATE EACH ERROR**:

// 1. **HEADER FIELD COUNT CHECK**: 
//    - Count how many header fields appear on the same line or consecutive lines
//    - If count = 1: ✅ Individual regex allowed (e.g., ""Grand Total: $166.30"")
//    - If count ≥ 2: 🚨 MUST use multi-field regex with ALL capture groups

// 2. **HEADER RESPONSE FORMAT VALIDATION**:
//    - ❌ REJECT: Multiple separate errors for header fields from the same line
//    - ✅ ACCEPT: One comprehensive error with multiple capture groups for same line
//    - ❌ REJECT: """"field"""": """"InvoiceNo"""" + separate """"field"""": """"InvoiceDate"""" from same line
//    - ✅ ACCEPT: """"field"""": """"ShipmentInvoice_MultiField_Line12"""" with captured_fields: [""""InvoiceNo"""", """"InvoiceDate""""]

// **HEADER CORRECT APPROACH**: 
// - Multiple header fields on same line → ONE comprehensive regex with ALL capture groups
// - Use field name pattern: ""ShipmentInvoice_MultiField_LineX""
// - Include captured_fields array listing ALL field names

// **HEADER INCORRECT APPROACH**:
// - Multiple separate regexes for header fields from the same line
// - Individual field names like ""InvoiceNo"" and ""InvoiceDate"" when both appear on same line

// **IF YOU SUBMIT INDIVIDUAL REGEXES FOR HEADER FIELDS FROM THE SAME LINE, YOUR RESPONSE IS INCORRECT AND WILL BE REJECTED**

// "If you find no new omissions or corrections, return an empty errors array.";

//             _logger.Information("🏁 **PROMPT_CREATION_COMPLETE (V13.0)**: Object-oriented business entity analysis prompt created with grouped and independent field architecture. Length: {PromptLength} characters.", prompt.Length);
//             return prompt;
//         }
...
//             // =============================== JSON ESCAPING FOR EXAMPLES ===============================
//             // Using same pattern as header prompt - escape regex patterns for safe JSON embedding
//             string jsonSafeQuantityRegex = EscapeRegexForJson(@"(?<Quantity>\d+)");
//             string jsonSafeItemDescRegex = EscapeRegexForJson(@"(?<ItemDescription>[A-Za-z\s\d]+)");
//             string jsonSafeUnitPriceRegex = EscapeRegexForJson(@"(?<UnitPrice>[\d.]+)");
//             string jsonSafeMultiFieldExample = EscapeRegexForJson(@"(?<Quantity>\d+)\s+of:\s+(?<ItemDescription>.+?)\s*[\$](?<UnitPrice>[\d.]+)");
//             _logger.Debug("   - **JSON_SAFE_REGEX_PREPARATION**: Escaped regex patterns for JSON examples prepared");
...
//                 "**GENERAL OBJECT-AWARE PATTERNS**:" + Environment.NewLine +
//                 "Use .+? for flexible text capture within object boundaries." + Environment.NewLine +
//                 "Use \\\\d+ for numbers, [A-Za-z\\\\s]+ for text patterns." + Environment.NewLine +
//                 "Design patterns to capture complete business objects, not fragments." + Environment.NewLine +
//                 "Prioritize object completeness over pattern simplicity." + Environment.NewLine + Environment.NewLine +
//                 "**🔍 MANDATORY REGEX VALIDATION AND MULTI-LINE REQUIREMENTS**:" + Environment.NewLine +
//                 "Before submitting ANY regex pattern, you MUST:" + Environment.NewLine + Environment.NewLine +
...
//                 "**2. MULTI-LINE FIELD APPEND STRATEGY**:" + Environment.NewLine +
//                 "   For ItemDescription spanning multiple lines, use NUMBERED CAPTURE GROUPS:" + Environment.NewLine +
//                 "   - First line: (?<ItemDescription>first line text)" + Environment.NewLine +
//                 "   - Second line: (?<ItemDescription2>second line text)" + Environment.NewLine +
//                 "   - Third line: (?<ItemDescription3>third line text)" + Environment.NewLine +
//                 "   - Production will create ItemDescription2, ItemDescription3 fields with Append=true" + Environment.NewLine + Environment.NewLine +
//                 "**3. PATTERN TESTING VALIDATION**:" + Environment.NewLine +
...
//                 "**MULTI-LINE EXAMPLES**:" + Environment.NewLine + Environment.NewLine +
//                 "**❌ WRONG SINGLE-LINE APPROACH**:" + Environment.NewLine +
//                 "Pattern: \"(?<Quantity>\\\\d+)\\\\s*of:\\\\s*(?<ItemDescription>.+?)\\\\$(?<UnitPrice>\\\\d+\\\\.\\\\d{2})\"" + Environment.NewLine +
//                 "RequiresMultilineRegex: false" + Environment.NewLine +
//                 "Applied to: \"3 of: PRODUCT NAME\\\\nCONTINUATION $39.99\"" + Environment.NewLine +
//                 "Problem: .+? stops at newline, only captures \"PRODUCT NAME\", misses \"CONTINUATION\"" + Environment.NewLine + Environment.NewLine +
//                 "**✅ CORRECT SINGLE-REGEX APPROACH**:" + Environment.NewLine +
//                 "Pattern: \"(?<Quantity>\\\\d+)\\\\s*of:\\\\s*(?<ItemDescription>.+?)\\\\s*\\\\$(?<UnitPrice>\\\\d+\\\\.\\\\d{2})\"" + Environment.NewLine +
//                 "RequiresMultilineRegex: true" + Environment.NewLine +
//                 "Applied to: \"3 of: PRODUCT NAME\\\\nCONTINUATION $39.99\"" + Environment.NewLine +
//                 "Result: With Singleline mode, .+? captures \"PRODUCT NAME\\\\nCONTINUATION\" completely" + Environment.NewLine + Environment.NewLine +
//                 "**✅ CORRECT NUMBERED-GROUPS APPROACH (Alternative)**:" + Environment.NewLine +
//                 "Pattern: \"(?<Quantity>\\\\d+)\\\\s*of:\\\\s*(?<ItemDescription>[^\\\\r\\\\n]+)[\\\\r\\\\n\\\\s]+(?<ItemDescription2>[^\\\\$]+)\\\\$(?<UnitPrice>\\\\d+\\\\.\\\\d{2})\"" + Environment.NewLine +
//                 "RequiresMultilineRegex: true" + Environment.NewLine +
//                 "Applied to: \"3 of: PRODUCT NAME\\\\nCONTINUATION $39.99\"" + Environment.NewLine +
//                 "Result: ItemDescription=\"PRODUCT NAME\", ItemDescription2=\"CONTINUATION\" (appends with space)" + Environment.NewLine +
//                 "captured_fields: [\"Quantity\", \"ItemDescription\", \"ItemDescription2\", \"UnitPrice\"]" + Environment.NewLine + Environment.NewLine +
//                 "**CRITICAL RULES**:" + Environment.NewLine +
//                 "1. If LineText contains \\\\n, you MUST set RequiresMultilineRegex: true" + Environment.NewLine +
//                 "2. Choose single .+? OR numbered groups - both work but serve different needs" + Environment.NewLine +
//                 "3. Test your pattern mentally before submitting - if it can't extract CorrectValue, fix it!" + Environment.NewLine +
//                 "4. Include ALL numbered groups in captured_fields array" + Environment.NewLine + Environment.NewLine +
//                 "**OCR CHARACTER CONFUSION HANDLING**:" + Environment.NewLine +
