    ...
    public class OCRLlmClient : IDisposable
    {
        ...
        private async Task<string> CallGeminiAsync(string prompt, double temperature, int maxTokens, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrEmpty(content))
            {
                ...
                // Determine document type using DatabaseTemplateHelper (MANDATORY - NO HARDCODING)
                string documentTypeGemini = "Shipment Invoice"; // Gemini API integration is document-type agnostic
        ...
        private HttpClient CreateHttpClient()
        {
            ...
            // Determine document type using DatabaseTemplateHelper (MANDATORY - NO HARDCODING)
            string documentType = "Shipment Invoice"; // HTTP client creation is document-type agnostic
            _logger.Error($"📋 **DOCUMENT_TYPE_DETECTED**: {documentType} - Using DatabaseTemplateHelper document-specific validation rules");
        ...
        private AsyncRetryPolicy CreateRetryPolicy()
        {
            ...
            // Determine document type using DatabaseTemplateHelper (MANDATORY - NO HARDCODING)
            string documentType = "Shipment Invoice"; // Retry policy creation is document-type agnostic
            _logger.Error($"📋 **DOCUMENT_TYPE_DETECTED**: {documentType} - Using DatabaseTemplateHelper document-specific validation rules");
