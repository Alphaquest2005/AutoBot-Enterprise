            ...
            foreach (var template in context.Templates)
            {
                ...
                try
                {
                    ...
                    if (fileType != null)
                    {
                        ...
                        _logger.Verbose(
                            "Retrieved FileType Description: '{FileDescription}' for FileTypeId: {FileTypeId}",
                ...
                {
                    _logger.Error(ex,
                        "Error getting FileType description from FileTypeManager for FileTypeId: {FileTypeId}",
                        template.OcrInvoices.FileTypeId);
                ...
                try
                {
                    ...
                    {
                        _logger.Information(
                            "Added entry to Imports dictionary with Key: {ImportKey}, Status: {ImportStatus}",
                            importKey, importStatus);
                    }
                }
                catch (Exception ex)
                {
                    _logger.Error(ex, "Error adding/updating entry in Imports dictionary for Key: {ImportKey}",
                        importKey);
