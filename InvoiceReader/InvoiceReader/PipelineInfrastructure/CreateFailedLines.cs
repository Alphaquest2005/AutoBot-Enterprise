...
public static partial class InvoiceProcessingUtils
{
    ...
    private static OCR_FailedLines CreateFailedLines(AsycudaDocumentSet_Attachments att, Line x)
    {
        ...
        if (x == null || x.OCR_Lines == null)
        {
            ...
            _utilsLogger.Error("CreateFailedLines called with null Line or null Line.OCR_Lines for AttachmentId: {AttachmentId}.", att?.Id ?? -1);
            return null; // Or throw
        }

        int lineId = x.OCR_Lines.Id; // Cache for logging
        _utilsLogger.Debug("Creating FailedLines object for AttachmentId: {AttachmentId}, LineId: {LineId}", att.Id, lineId);
        ...
        try
        {
            ...
            if (x.FailedFields != null)
            {
                 ...
                 _utilsLogger.Verbose("Found {DistinctFieldCount} distinct failed fields for LineId: {LineId}", distinctFields.Count, lineId);
                 ...
                 failedFieldsList = distinctFields
                    ...
                    .ToList();
                 _utilsLogger.Verbose("Created {FailedFieldObjectCount} OCR_FailedFields objects for LineId: {LineId}", failedFieldsList.Count, lineId);
            }
            else
            {
            ...
            _utilsLogger.Debug("Successfully created OCR_FailedLines object for AttachmentId: {AttachmentId}, LineId: {LineId} with {FailedFieldCount} associated fields.",
                att.Id, lineId, failedFieldsList.Count);
        }
        catch (Exception ex)
        {
             _utilsLogger.Error(ex, "Error creating OCR_FailedLines for AttachmentId: {AttachmentId}, LineId: {LineId}", att.Id, lineId);
