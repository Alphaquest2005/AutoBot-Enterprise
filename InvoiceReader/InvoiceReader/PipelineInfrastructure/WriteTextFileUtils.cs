...
namespace WaterNut.DataSpace.PipelineInfrastructure
{
    public static partial class InvoiceProcessingUtils
    {
        ...
        public static string WriteTextFile(string file, string pdftxt, ILogger logger)
        {
             logger.Debug("WriteTextFile called for base file: {BaseFilePath}", file);

             // Null/Empty checks for inputs
             if (string.IsNullOrEmpty(file))
             {
                  logger.Error("WriteTextFile cannot proceed: base file path is null or empty.");
             ...
             if (pdftxt == null)
             {
                  logger.Warning("WriteTextFile called with null pdftxt for base file: {BaseFilePath}. An empty file will be created/overwritten.", file);
             ...
             {
                 txtFile = file + ".txt";
                 logger.Information("Writing text (Length: {Length}) to file: {TxtFilePath}", pdftxt.Length, txtFile);
                 ...
                 logger.Information("Successfully wrote text to {TxtFilePath}", txtFile);
                 return txtFile;
             }
             catch (UnauthorizedAccessException uaEx) // Specific exception
             {
                  logger.Error(uaEx, "Unauthorized access error writing text file: {TxtFilePath}", txtFile ?? file + ".txt");
                  return null; // Indicate failure
             }
             catch (DirectoryNotFoundException dnfEx) // Specific exception
             {
                  logger.Error(dnfEx, "Directory not found error writing text file: {TxtFilePath}", txtFile ?? file + ".txt");
                  return null; // Indicate failure
             }
             catch (IOException ioEx) // Catch specific IO errors
             {
                  logger.Error(ioEx, "IO Error writing text file: {TxtFilePath}", txtFile ?? file + ".txt");
                  return null; // Indicate failure
             }
             catch (Exception ex) // Catch other potential errors
             {
                  logger.Error(ex, "Error during WriteTextFile for base file: {BaseFilePath}", file);
                  return null; // Indicate failure
             }
        }
    }
