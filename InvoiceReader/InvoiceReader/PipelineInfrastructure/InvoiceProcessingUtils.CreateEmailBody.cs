    ...
    private static string CreateEmailBody(string error, List<Line> failedlst, FileInfo fileInfo)
    {
        ...
        if (failedlst != null && failedlst.Any())
        {
            ...
            try
            {
                ...
                {
                    _utilsLogger.Verbose("Could not extract first invoice name from failed lines for File: {FileName}",
                        fileName);
                }
                ...
                failedLinesInfo = failedlst
                        ...
                        try
                        {
                            if (line.FailedFields != null)
                            {
                                ...
                                var details = line.FailedFields
                                    ...
                                    .SelectMany(list =>
                                        list ?? Enumerable.Empty<KeyValuePair<(Fields fields, int instance), string>>())
                    ...
                    .Aggregate((o, c) => o + "\r\n" + c);
                _utilsLogger.Verbose("Constructed failed lines details string (Length: {Length}) for File: {FileName}",
