...
public static partial class InvoiceProcessingUtils
{
    ...
    {
         _utilsLogger.Debug("Attempting to create DocSet Attachment for File: {FilePath}, EmailId: {EmailId}, FileTypeId: {FileTypeId}, DocSetId: {DocSetId}",
            file, emailId, fileTypeId, docSet?.AsycudaDocumentSetId ?? -1); // Added null check for docSet

        if (docSet == null)
        {
            _utilsLogger.Error("Cannot create attachment because AsycudaDocumentSet is null for File: {FilePath}", file);
        ...
        try
        {
            ...
            {
                  _utilsLogger.Verbose("Creating new AsycudaDocumentSet_Attachments object in memory for File: {FilePath}", file);
                ...
                ctx.AsycudaDocumentSet_Attachments.Add(newAttachment);
                  _utilsLogger.Debug("Saving new attachment to database for File: {FilePath}, DocSetId: {DocSetId}", file, docSet.AsycudaDocumentSetId);
                ctx.SaveChanges();
                  // Assuming AsycudaDocumentSet_Attachments has an 'Id' property populated after SaveChanges
                  _utilsLogger.Information("Successfully created and saved DocSet Attachment with Id: {AttachmentId} for File: {FilePath}, DocSetId: {DocSetId}",
                    newAttachment?.Id ?? -1, file, docSet.AsycudaDocumentSetId);
            }
        }
        catch (Exception ex)
        {
            _utilsLogger.Error(ex, "Failed to create or save DocSet Attachment for File: {FilePath}, EmailId: {EmailId}, DocSetId: {DocSetId}",
