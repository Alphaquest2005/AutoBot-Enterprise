    ...
    public partial class InvoiceProcessingPipeline
    {
        public async Task<bool> RunPipeline()
        {
            ...
            try
            {
                ...
                _logger.Verbose("Initial steps: {@InitialSteps}",
                    initialSteps.Select(step => step.GetType().Name)); // Log step types
                ...
                _logger.Information("Initial pipeline steps completed for File: {FilePath}. Success: {Success}",
                    filePath,
                    initialRunSuccess);
                ...
                _logger.Debug("IsInitialRunUnsuccessful result for File: {FilePath}: {Result}", filePath,
                    isInitialRunUnsuccessful);
                ...
                if (isInitialRunUnsuccessful)
                {
                    ...
                    _logger.Information(
                        "Error pipeline processing finished for File: {FilePath}. Result (Continue?): {Result}",
                        filePath,
                        errorPipelineResult);
                    _logger.Verbose("Context after error pipeline: {@Context}",
                        _context); // Log context after error pipeline
                ...
                else
                {
                    ...
                    _logger.Verbose("Context after success pipeline: {@Context}",
                        _context); // Log context after success pipeline
            ...
            {
                _logger.Information(
                    "Finished main InvoiceProcessingPipeline execution for File: {FilePath}. Final Import Status: {ImportStatus}",
