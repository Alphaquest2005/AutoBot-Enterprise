            ...
            AsycudaDocumentSet_Attachments att)
        {
            ...
            try
            {
                ...
                _utilsLogger.Verbose(
                    "ImportErrors object created in memory with Id: {ImportErrorId} (matching AttachmentId)",
                    importErr.Id);
                ...
                _utilsLogger.Information(
                    "Successfully created new ImportErrors object for AttachmentId: {AttachmentId}", attachmentId);
            }
            catch (Exception ex)
            {
                _utilsLogger.Error(ex, "Error creating new ImportErrors for AttachmentId: {AttachmentId}",
                    attachmentId);
