            ...
            AsycudaDocumentSet_Attachments att)
        {
            ...
            if (att == null)
            {
                _utilsLogger.Warning(
                    "UpdateImportError called with null AsycudaDocumentSet_Attachments object for ImportErrors Id: {ImportErrorId}. Cannot create/update failed lines.",
                    importErr.Id);
            }
            ...
            {
                _utilsLogger.Warning(
                    "UpdateImportError called with null failed lines list for ImportErrors Id: {ImportErrorId}. OCR_FailedLines will be empty.",
                    importErr.Id);
            ...
            try
            {
                ...
                _utilsLogger.Verbose("Updated ImportErrors Id: {ImportErrorId} - PdfText Length: {Length}",
                    importErrorId, pdftxt?.Length ?? 0);

                importErr.Error = error;
                _utilsLogger.Verbose("Updated ImportErrors Id: {ImportErrorId} - Error: {Error}", importErrorId,
                    error); // Be cautious logging full error if it contains sensitive info

                importErr.EntryDateTime = DateTime.Now;
                _utilsLogger.Verbose("Updated ImportErrors Id: {ImportErrorId} - EntryDateTime set to: {DateTime}",
                    importErrorId, importErr.EntryDateTime);

                // Process failed lines only if attachment is not null
                if (att != null)
                {
                    _utilsLogger.Debug(
                        "Processing {FailedLineCount} failed lines to create/update OCR_FailedLines for ImportErrors Id: {ImportErrorId}",
                        failedlst.Count, importErrorId);
                    ...
                    _utilsLogger.Information(
                        "Updated ImportErrors Id: {ImportErrorId} - Replaced OCR_FailedLines collection with {FailedLinesCount} new entries.",
                        importErrorId, importErr.OCR_FailedLines?.Count ?? 0);
                }
                else
                {
                    _utilsLogger.Warning(
                        "Skipped processing OCR_FailedLines for ImportErrors Id: {ImportErrorId} because attachment was null.",
                        importErrorId);
