...
public static partial class InvoiceProcessingUtils
{
        ...
        List<AsycudaDocumentSet_Attachments> existingAttachment)
    {
        ...
        if (existingAttachment == null)
        {
         ...
         if (failedlst == null)
        {
             _utilsLogger.Warning("SaveDbErrorDetails called with null failed lines list. Error details might be incomplete.");
    ...
    private static void ProcessImportError(string pdftxt, string error, List<Line> failedlst,  AsycudaDocumentSet_Attachments att)
    {
        ...
        try
        {
            using (var ctx = new OCRContext()) // Assuming OCRContext is accessible and disposable
            {
                ...
                if (importErr == null)
                {
                    _utilsLogger.Information("No existing ImportErrors found for Id: {AttachmentId}. Creating new entry.", attachmentId);
                    // CreateNewImportErrors handles its own logging
                    importErr = CreateNewImportErrors(pdftxt, error, failedlst, att);
                    if (importErr != null)
                    {
                         _utilsLogger.Debug("Adding new ImportErrors (Id: {ImportErrorId}) to context.", importErr.Id);
                         ctx.ImportErrors.Add(importErr);
                    } else {
                         _utilsLogger.Error("CreateNewImportErrors returned null for AttachmentId: {AttachmentId}. Cannot add error.", attachmentId);
                         return; // Don't proceed if creation failed
                    }
                }
                else
                {
                    _utilsLogger.Information("Existing ImportErrors found for Id: {AttachmentId}. Updating entry.", attachmentId);
                ...
                ctx.SaveChanges();
                _utilsLogger.Information("Successfully saved ImportErrors details for AttachmentId: {AttachmentId}", attachmentId);
            } // Context disposed here
        }
        catch (Exception ex) // Catch context creation or SaveChanges errors
        {
             _utilsLogger.Error(ex, "Error during ProcessImportError (Context creation or SaveChanges) for AttachmentId: {AttachmentId}", attachmentId);
