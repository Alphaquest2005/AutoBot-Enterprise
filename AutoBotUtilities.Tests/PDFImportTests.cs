...
namespace AutoBotUtilities.Tests
{
    ...
    public class PDFImportTests
    {
        ...
        [Test]
        public void CanImportAmazonMultiSectionInvoice() 
        {
            try
            {
                ...
                var fileTypes = (IEnumerable<CoreEntities.Business.Entities.FileTypes>)FileTypeManager.GetImportableFileType(FileTypeManager.EntryTypes.ShipmentInvoice, FileTypeManager.FileFormats.PDF, testFile); // Fully qualified
                if (!fileTypes.Any())
                {
        ...
        [Test]
        public void CanImportSheinMultiInvoice() 
        {
            try
            {
                //if (!Infrastructure.Utils.IsTestApplicationSettings()) Assert.That(true); // Skip if not test settings
