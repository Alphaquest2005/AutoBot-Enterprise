...
namespace AutoBotUtilities.Tests
{
    ...
    {
        private static ILogger _log; // Changed to ILogger
        private string invocationId; // Field to hold the unique invocation ID for each test run
        ...
        public void OneTimeSetup()
        {
            ...
            _log = new LoggerConfiguration()
                ...
                .Enrich.FromLogContext() // Ensure LogContext properties are enriched
                .WriteTo.Console()
                .WriteTo.NUnitOutput()
                .WriteTo.File("EmailDownloaderIntegrationTests.log",
                    rollingInterval: RollingInterval.Day,
                    outputTemplate: "{Timestamp:yyyy-MM-dd HH:mm:ss.fff zzz} [{Level:u3}] [{SourceContext}] {Message:lj}{NewLine}{Exception}")
                .CreateLogger();
            Log.Logger = _log; // Assign to the static Log.Logger for global access if other parts of test/system rely on it

            // Note: 'invocationId' field is not set here, so passing null or relying on default for extensions
            ...
            try
            {
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    if (testAppSettingFromDb != null)
                    {
                        ...
                        _log.LogInfoCategorized(LogCategory.Undefined, "Successfully loaded Email/Password for AppSettingId: {AppSettingId} (Email: {EmailAddress}) from database.", invocationId: null, _dbConfiguredApplicationSettingsId, _dbConfiguredEmailAddress);
                    }
                    else
                    {
            ...
            try
            {
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "Derived IMAP: {ImapServer}:{ImapPort} ({ImapOptions})", invocationId: null, _resolvedImapServer, _resolvedImapPort, _resolvedImapOptions);
                _log.LogInfoCategorized(LogCategory.Undefined, "Derived SMTP: {SmtpServer}:{SmtpPort} ({SmtpOptions})", invocationId: null, _resolvedSmtpServer, _resolvedSmtpPort, _resolvedSmtpOptions);
            }
            catch (Exception ex)
            {
            ...
            _log.LogInfoCategorized(LogCategory.Undefined, "Test Data Folder: {TestDataFolder}", invocationId: null, _testDataFolder);
            ...
            if (testAppSettingFromDb?.EmailMapping != null)
            {
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "Added {Count} EmailMappings from database AppSettingId: {AppSettingId}", invocationId: null, testAppSettingFromDb.EmailMapping.Count, _dbConfiguredApplicationSettingsId);
            ...
            {
                _log.LogWarningCategorized(LogCategory.Undefined, "No EmailMappings found in the loaded ApplicationSettings (ID: {AppSettingId}). Adding default mappings for tests.", invocationId: null, _dbConfiguredApplicationSettingsId);
            ...
            _log.LogInfoCategorized(LogCategory.Undefined, "Test client configured for EmailDownloader operations using AppSettingId: {AppSettingId}", invocationId: null, _testClientForDownloader.ApplicationSettingsId);
        ...
        {
            // Note: 'invocationId' field is not set here by SetupForEachTest
            ...
            if (Directory.Exists(_testDataFolder))
            {
                try
                {
                    ...
                    _log.LogInfoCategorized(LogCategory.Undefined, "Deleted test data folder: {TestDataFolder}", invocationId: null, _testDataFolder);
                }
                catch (Exception ex) { _log.LogWarningCategorized(LogCategory.Undefined, ex, "Error deleting test folder {TestDataFolder}", invocationId: null, _testDataFolder); }
            }
            Log.CloseAndFlush(); // Static Serilog method, leave as is
            Console.WriteLine("=== EmailDownloaderIntegrationTests OneTimeTearDown Complete ==="); // Non-Serilog, leave as is
        }
        ...
        public void SetupForEachTest()
        {
            ...
            // Log entry here would be LogMethodEntry, but it's a setup method.
            // Let's use a specific category or internal step for test setup.
            _log.LogInfoCategorized(LogCategory.InternalStep, "--- Test Setup Starting: {TestName} ---", invocationId: this.invocationId, TestContext.CurrentContext.Test.Name);
            EmailDownloader.EmailDownloader.ReturnOnlyUnknownMails = false;
            _testClientForDownloader.NotifyUnknownMessages = true;
            _log.LogInfoCategorized(LogCategory.InternalStep, "--- Test Setup Complete: {TestName} ---", invocationId: this.invocationId, TestContext.CurrentContext.Test.Name);
        }

        private async Task SendTestEmailAsync(string subject, string body, string[] attachments = null, string toAddress = null)
        {
            // This is a helper method, 'this.invocationId' from the test method will be in LogContext
            // if the logger instance _log captures it, or if we pass it explicitly.
            // The extensions now use LogContext.PushProperty for invocationId.
            ...
            {
                if (File.Exists(attPath)) { bodyBuilder.Attachments.Add(attPath); _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Adding attachment: {AttachmentPath}", this.invocationId, attPath); }
                else _log.LogWarningCategorized(LogCategory.Undefined, "Test setup warning: Attachment not found, skipping: {AttachmentPath}", this.invocationId, attPath);
            }
            ...
            using (var smtpClient = new SmtpClient())
            {
                try
                {
                    _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Connecting SMTP: {SmtpServer}:{SmtpPort} ({SmtpOptions})", this.invocationId, _resolvedSmtpServer, _resolvedSmtpPort, _resolvedSmtpOptions);
                    await smtpClient.ConnectAsync(_resolvedSmtpServer, _resolvedSmtpPort, _resolvedSmtpOptions).ConfigureAwait(false);
                    _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Authenticating SMTP: {EmailAddress}", this.invocationId, _dbConfiguredEmailAddress);
                    await smtpClient.AuthenticateAsync(_dbConfiguredEmailAddress, _dbConfiguredEmailPassword).ConfigureAwait(false);
                    _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Sending SMTP: To={ToAddress}, Subject={Subject}", this.invocationId, toAddress, subject);
                    await smtpClient.SendAsync(message).ConfigureAwait(false);
                    await smtpClient.DisconnectAsync(true).ConfigureAwait(false);
                    _log.LogInfoCategorized(LogCategory.Undefined, "Sent test email. Subject: '{Subject}' To: '{ToAddress}'", this.invocationId, subject, toAddress);
                }
                catch (Exception ex) { _log.LogErrorCategorized(LogCategory.Undefined, ex, "Failed to send test email. Subject: {Subject}, To: {ToAddress}", this.invocationId, subject, toAddress); throw; }
            }
        ...
        private async Task<ImapClient> GetTestImapClientAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Connecting IMAP: {ImapServer}:{ImapPort} ({ImapOptions})", this.invocationId, _resolvedImapServer, _resolvedImapPort, _resolvedImapOptions);
                ...
                _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Authenticating IMAP: {EmailAddress}", this.invocationId, _dbConfiguredEmailAddress);
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "IMAP client connected & Inbox opened: {EmailAddress}", this.invocationId, _dbConfiguredEmailAddress);
                return imapClient;
            }
            catch (Exception e)
            {
                _log.LogErrorCategorized(LogCategory.Undefined, e, "GetTestImapClientAsync Error for {EmailAddress} on {ImapServer}", this.invocationId, _dbConfiguredEmailAddress, _resolvedImapServer);
        ...
        {
            _log.LogMethodEntry(this.invocationId); // Caller info will be A_CanSendAndReceiveBasicEmail_StreamEmailResultsAsync
            ...
            try
            {
                ...
                _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Starting StreamEmailResultsAsync to find email: {Subject}", this.invocationId, testSubject);
                ...
                foreach (var emailTask in streamEmailResultsAsync)
                {
                    ...
                    if (result != null)
                    {
                        _log.LogVerboseCategorized(LogCategory.DiagnosticDetail, "StreamEmailResultsAsync yielded: Subject='{Subject}', UID={Uid}", this.invocationId, result.EmailKey.EmailMessage?.Subject, result.EmailKey.UidString);
                        if (result.EmailKey.EmailMessage?.Subject == testSubject)
                        {
                            _log.LogInfoCategorized(LogCategory.Undefined, "Found matching email: {Subject}", this.invocationId, testSubject);
            ...
            {
                _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error during StreamEmailResultsAsync processing in test A.", this.invocationId);
            ...
            finally
            {
                ...
                {
                    _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Disconnecting test IMAP client.", this.invocationId);
            ...
            _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Checking for Info.txt at: {InfoTxtPath}", this.invocationId, infoTxtPath);
            ...
            stopwatch.Stop();
            _log.LogMethodExitSuccess(this.invocationId, stopwatch.ElapsedMilliseconds);
        }
        ...
        {
            // InvocationId is initialized in [Setup] for this test method
            var stopwatch = Stopwatch.StartNew(); // Keep stopwatch local to the test method
            // LogContext.PushProperty("InvocationId", this.invocationId); // Already handled by extensions
            ...
            try
            {
                ...
                _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Verifying test PDF file exists at {FilePath}", this.invocationId, TestPdfFilePath);
                Assert.That(File.Exists(TestPdfFilePath), Is.True, $"Test PDF file not found at: {TestPdfFilePath}");
                _log.LogInfoCategorized(LogCategory.InternalStep, "Verified test PDF file existence; FileName: {FileName}; FileExists: {FileExists}", this.invocationId, TestPdfFilePath, File.Exists(TestPdfFilePath));
                ...
                string uniqueContent = Guid.NewGuid().ToString();
                string testSubject = $"PDF Import Test {uniqueContent}";
                string testBody = "Integration test email with PDF attachment for ProcessEmailsAsync.";
                string invoiceNumberToVerify = "114-7827932-2029910";

                _log.LogInfoCategorized(LogCategory.InternalStep, "Starting database cleanup for EntryDataDS; TargetInvoiceNumber: {TargetInvoiceNumber}", this.invocationId, invoiceNumberToVerify);
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "Cleaning up previous test data for InvoiceNo: {InvoiceNumber} in EntryDataDS...", this.invocationId, invoiceNumberToVerify);
                try
                {
                    using (var entryCtx = new EntryDataDSContext())
                    {
                        ...
                        {
                            _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Removing {Count} existing ShipmentInvoiceDetails.", this.invocationId, existingDetails.Count);
                        ...
                        {
                            _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Removing existing ShipmentInvoice.", this.invocationId);
                            entryCtx.ShipmentInvoice.Remove(existingInvoice);
                        }
                        await entryCtx.SaveChangesAsync().ConfigureAwait(false);
                        _log.LogInfoCategorized(LogCategory.Undefined, "EntryDataDS cleanup complete.", this.invocationId);
                    }
                }
                catch (Exception ex)
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error during EntryDataDS database cleanup.", this.invocationId);
                    Assert.Fail($"Database cleanup failed: {ex.Message}");
                }
                _log.LogInfoCategorized(LogCategory.InternalStep, "Finished database cleanup for EntryDataDS; TargetInvoiceNumber: {TargetInvoiceNumber}", this.invocationId, invoiceNumberToVerify);
                ...
                _log.LogInfoCategorized(LogCategory.InternalStep, "Preparing to send test email with PDF attachment; EmailSubject: {EmailSubject}; AttachmentsCount: {AttachmentsCount}", this.invocationId, testSubject, 1);
                ...
                UniqueId targetEmailUid = UniqueId.Invalid;
                ...
                try
                {
                    ...
                    foreach (var emailTask in streamEmailResultsAsync)
                    {
                        ...
                        if (result != null && result.EmailKey.EmailMessage?.Subject == testSubject)
                        {
                            ...
                            _log.LogInfoCategorized(LogCategory.Undefined, "Found and processed test email during initial download simulation: Subject='{Subject}', UID={Uid}", this.invocationId, testSubject, result.EmailKey.UidString);

                            if (!UniqueId.TryParse(result.EmailKey.UidString, out targetEmailUid))
                            {
                                _log.LogErrorCategorized(LogCategory.Undefined, "Failed to parse UID string '{UidString}' from processed email result.", this.invocationId, result.EmailKey.UidString);
                                Assert.Fail($"Could not parse UID '{result.EmailKey.UidString}' needed for the test.");
                            }
                            _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Successfully parsed target email UID: {Uid}", this.invocationId, targetEmailUid);
                            ...
                            else _log.LogWarningCategorized(LogCategory.Undefined, "Could not simulate storing email in DB due to missing EmailMessage or UidString.", this.invocationId);
                            ...
                            _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Checking for downloaded PDF at: {PdfPath}", this.invocationId, downloadedPdfPath);
                            Assert.That(File.Exists(downloadedPdfPath), Is.True, $"Downloaded PDF not found at expected path: {downloadedPdfPath}.");
                            _log.LogInfoCategorized(LogCategory.Undefined, "Verified downloaded PDF exists at: {PdfPath}", this.invocationId, downloadedPdfPath);
                ...
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error during initial email download simulation phase.", this.invocationId);
                ...
                finally
                {
                    ...
                    {
                        _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Disconnecting IMAP client used for initial download simulation.", this.invocationId);
                ...
                _log.LogInfoCategorized(LogCategory.InternalStep, "Finished simulation of initial email download; EmailSubject: {EmailSubject}; TargetEmailUid: {TargetEmailUid}; DownloadedPdfPath: {DownloadedPdfPath}", this.invocationId, testSubject, targetEmailUid.ToString(), downloadedPdfPath);
                ...
                _log.LogInfoCategorized(LogCategory.InternalStep, "Starting to mark email as UNREAD; TargetEmailUid: {TargetEmailUid}", this.invocationId, targetEmailUid.ToString());
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "Attempting to mark email UID {Uid} as UNREAD before calling ProcessEmailsAsync.", this.invocationId, targetEmailUid);
                ...
                try
                {
                    ...
                    await markUnreadClient.Inbox.RemoveFlagsAsync(targetEmailUid, MessageFlags.Seen, true, CancellationToken.None).ConfigureAwait(false);
                    _log.LogInfoCategorized(LogCategory.Undefined, "Successfully marked email UID {Uid} as UNREAD.", this.invocationId, targetEmailUid);
                    await markUnreadClient.DisconnectAsync(true, CancellationToken.None).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Failed to mark email UID {Uid} as UNREAD.", this.invocationId, targetEmailUid);
                ...
                {
                    markUnreadClient?.Dispose();
                }
                _log.LogInfoCategorized(LogCategory.InternalStep, "Finished marking email as UNREAD; TargetEmailUid: {TargetEmailUid}; MarkedAsUnread: {MarkedAsUnread}", this.invocationId, targetEmailUid.ToString(), true);
                ...
                _log.LogInfoCategorized(LogCategory.InternalStep, "Starting preparation for ProcessEmailsAsync call (loading ApplicationSettings); ApplicationSettingsId: {ApplicationSettingsId}", this.invocationId, _dbConfiguredApplicationSettingsId);
                ...
                try
                {
                    ...
                    _log.LogInfoCategorized(LogCategory.InternalStep, "Finished preparation for ProcessEmailsAsync call; ApplicationSettingsId: {ApplicationSettingsId}; AppSettingLoaded: {AppSettingLoaded}", this.invocationId, _dbConfiguredApplicationSettingsId, appSetting != null);
                    ...
                    _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Retrieved ApplicationSettings: ID={AppSettingId}, Name='{SoftwareName}'", this.invocationId, appSetting.ApplicationSettingsId, appSetting.SoftwareName);
                }
                catch (Exception ex)
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error retrieving ApplicationSettings for ProcessEmailsAsync.", this.invocationId);
                ...
                _log.LogInfoCategorized(LogCategory.InternalStep, "Preparing to call EmailProcessor.ProcessEmailsAsync; AppSettingId: {AppSettingId}; BeforeImportTime: {BeforeImportTime}", this.invocationId, appSetting.ApplicationSettingsId, beforeImport);
                ...
                _log.LogInfoCategorized(LogCategory.Undefined, "Calling EmailProcessor.ProcessEmailsAsync for AppSetting ID: {AppSettingId}...", this.invocationId, appSetting.ApplicationSettingsId);
                try
                {
                    ...
                    _log.LogInfoCategorized(LogCategory.InternalStep, "EmailProcessor.ProcessEmailsAsync finished; AppSettingId: {AppSettingId}", this.invocationId, appSetting.ApplicationSettingsId);
                ...
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Exception during EmailProcessor.ProcessEmailsAsync execution.", this.invocationId);
                    _log.LogActionStart(this.invocationId, "AssertPhase_ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest", parentAction: "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Execution");
                    _log.LogMetaDirective(LogEventLevel.Warning, "Instrumentation", "AssertPhase_Start", "Added ACTION_START for Assert phase.", "LLM_Iter_0.1_PlanStep_4.6_Phase2.3.1", expectedChange: "ACTION_START for AssertPhase logged.", invocationId: this.invocationId);
                ...
                try
                {
                    ...
                    {
                        _log.LogVerboseCategorized(LogCategory.DiagnosticDetail, "Checking for ShipmentInvoice '{InvoiceNumber}'", this.invocationId, invoiceNumberToVerify);
                        ...
                        _log.LogVerboseCategorized(LogCategory.DiagnosticDetail, "ShipmentInvoice found: {Exists}", this.invocationId, invoiceExists);

                        _log.LogVerboseCategorized(LogCategory.DiagnosticDetail, "Checking for ShipmentInvoiceDetails count > 2 for '{InvoiceNumber}'", this.invocationId, invoiceNumberToVerify);
                        ...
                        _log.LogVerboseCategorized(LogCategory.DiagnosticDetail, "ShipmentInvoiceDetails count: {Count}", this.invocationId, detailCount);
                        ...
                        _log.LogInfoCategorized(LogCategory.Undefined, "Verification successful. Total Invoices: {InvoiceCount}, Total Details: {DetailCount}", this.invocationId, totalInvoices, totalDetails);
                    }
                }
                catch (Exception ex)
                {
                    _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error during database verification in EntryDataDSContext.", this.invocationId);
            ...
            {
                stopwatch.Stop();
                _log.LogMethodExitFailure(this.invocationId, stopwatch.ElapsedMilliseconds, ex);
                _log.LogMetaDirective(LogEventLevel.Warning, "Instrumentation", "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Exit", "Added METHOD_EXIT_FAILURE log.", "LLM_Iter_0.1_PlanStep_4.6", expectedChange: "METHOD_EXIT_FAILURE logged on exception.", invocationId: this.invocationId);
                _log.LogActionEndFailure(this.invocationId, "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Execution", stopwatch.ElapsedMilliseconds, ex);
                _log.LogMetaDirective(LogEventLevel.Warning, "Instrumentation", "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Exit", "Added ACTION_END_FAILURE log.", "LLM_Iter_0.1_PlanStep_4.6", expectedChange: "ACTION_END_FAILURE logged on exception.", invocationId: this.invocationId);
                throw;
            }
            finally
            {
                ...
                _log.LogMethodExitSuccess(this.invocationId, stopwatch.ElapsedMilliseconds);
                _log.LogMetaDirective(LogEventLevel.Warning, "Instrumentation", "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Exit", "Added METHOD_EXIT_SUCCESS log.", "LLM_Iter_0.1_PlanStep_4.6", expectedChange: "METHOD_EXIT_SUCCESS logged on normal completion.", invocationId: this.invocationId);
                _log.LogActionEndSuccess(this.invocationId, "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Execution", stopwatch.ElapsedMilliseconds);
                _log.LogMetaDirective(LogEventLevel.Warning, "Instrumentation", "ProcessEmailsAsync_ImportsPdfFromEmail_IntegrationTest_Exit", "Added ACTION_END_SUCCESS log.", "LLM_Iter_0.1_PlanStep_4.6", expectedChange: "ACTION_END_SUCCESS logged on normal completion.", invocationId: this.invocationId);
            }
        }
            ...
            using (var dbCtx = new CoreEntitiesContext())
            {
                try
                {
                    ...
                    if (!exists)
                    {
                        ...
                        _log.LogInfoCategorized(LogCategory.InternalStep, "Simulated storing email in DB: Subject='{Subject}', AppSettingId={AppSettingId}, UID={Uid}", this.invocationId, emailData.Subject, appSettingsId, uidUint);
                    }
                    else _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Email already simulated in DB: Subject='{Subject}', AppSettingId={AppSettingId}, UID={Uid}", this.invocationId, emailData.Subject, appSettingsId, uidUint);
                }
        ...
        {
            // Assuming 'this.invocationId' is available
            if (emailData == null || string.IsNullOrEmpty(emailData.Subject)) { _log.LogWarningCategorized(LogCategory.Undefined, "SimulateStoringEmailInDbForForwarding: emailData or Subject is null/empty.", invocationId: this.invocationId); return; }
            if (string.IsNullOrEmpty(appSpecificEmailId)) { _log.LogWarningCategorized(LogCategory.Undefined, "SimulateStoringEmailInDbForForwarding: appSpecificEmailId is null or empty.", invocationId: this.invocationId); return; }
            ...
            using (var dbCtx = new CoreEntitiesContext())
            {
                try
                {
                    ...
                    if (!exists)
                    {
                        ...
                        dbCtx.Emails.Add(emailEntity);
                        await dbCtx.SaveChangesAsync().ConfigureAwait(false);
                        _log.LogInfoCategorized(LogCategory.InternalStep, "Simulated storing email for forwarding: AppEmailId='{AppEmailId}', AppSettingId={AppSettingId}, UID={Uid}", this.invocationId, appSpecificEmailId, appSettingsId, mailkitUid);
                    }
                    else _log.LogDebugCategorized(LogCategory.DiagnosticDetail, "Email for forwarding already simulated in DB: AppEmailId='{AppEmailId}', AppSettingId={AppSettingId}", this.invocationId, appSpecificEmailId, appSettingsId);
                }
                catch (Exception ex) { _log.LogErrorCategorized(LogCategory.Undefined, ex, "Error simulating storing email for forwarding with AppEmailId: {AppEmailId}", this.invocationId, appSpecificEmailId); }
            }
        }
    }
}