# Invoice Diagnostic Report - 03152025_Sign_Out

## 📋 **File Identification**
- **Invoice File**: 03152025_Sign_Out.txt
- **Test Version**: v1.1
- **Test Date**: 2025-06-26 22:09:53
- **Vendor Category**: Other
- **Issue Category**: MULTI_ORDER_CONFUSION

## 🎯 **Issue Summary**
- **Primary Issue**: MULTI_ORDER_CONFUSION
- **Severity**: HIGH
...
- **Financial Patterns**: Credit, Discount, Tax
- **Special Characteristics**: Multiple Orders
...
```json
{
  "extractedFields": {
    "InvoiceTotal": 26.48,
    "SubTotal": 21.97,
    "TotalInternalFreight": 2.99,
    "TotalOtherCost": 1.54,
    "TotalInsurance": 0.0,
    "TotalDeduction": 0.0
  },
  "validation": {
    "CalculatedTotal": 26.48,
    "BalanceCheck": 0.0,
...
### **Issue 1: Multiple Orders Causing Field Mixing**
- **Problem**: Document contains 2 orders, causing DeepSeek to mix financial values from different orders
- **Evidence**: Order count detected: 2, Multiple 'Order ID:' or 'Order total:' patterns found, Financial values may come from different order sections
- **Expected Behavior**: Should extract fields from first/primary order only
- **Actual Behavior**: DeepSeek selects values from multiple orders causing balance errors
- **Proposed Fix**:
```
Add multi-order detection logic to DeepSeek prompt:
1. Detect multiple order patterns
2. Extract from first order only
3. Ignore subsequent orders to prevent mixing
```

### **Issue 2: Actual Credit Pattern Not Mapped to TotalInsurance**
...
- 'Store Credit' → TotalInsurance
- 'Account Credit' → TotalInsurance
- 'Gift Card' → TotalInsurance
