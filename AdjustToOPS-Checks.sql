...
SELECT AdjustmentDetails.EntryDataId AS InvoiceNo, AdjustmentDetails.EffectiveDate, CASE WHEN AliasName IS NOT NULL THEN inventoryitemalias.itemnumber ELSE AdjustmentDetails.ItemNumber END AS ItemNumber, 
                 AdjustmentDetails.ItemDescription, SUM(ISNULL(AdjustmentDetails.ReceivedQty, 0) - ISNULL(AdjustmentDetails.InvoiceQty, 0)) AS Quantity, AVG(AdjustmentDetails.Cost) AS Cost, AdjustmentDetails.IsReconciled
into #adjmentsData
FROM    AsycudaDocumentItemEntryDataDetails  with (nolock)RIGHT OUTER JOIN
                 AdjustmentDetails with (nolock) LEFT OUTER JOIN
                 InventoryItemAlias with (nolock) ON AdjustmentDetails.ItemNumber = InventoryItemAlias.AliasName ON AsycudaDocumentItemEntryDataDetails.EntryDataDetailsId = AdjustmentDetails.EntryDataDetailsId
WHERE (AdjustmentDetails.EffectiveDate BETWEEN @startdate AND @endDate) AND (AsycudaDocumentItemEntryDataDetails.EntryDataDetailsId IS NULL or 
					( AsycudaDocumentItemEntryDataDetails.EntryDataDetailsId IS NULL and isnull(AdjustmentDetails.IsReconciled, 0) = 1))
GROUP BY CASE WHEN AliasName IS NOT NULL THEN inventoryitemalias.itemnumber ELSE AdjustmentDetails.ItemNumber END, AdjustmentDetails.ItemDescription, AdjustmentDetails.EntryDataId, 
                 AdjustmentDetails.EffectiveDate,  AdjustmentDetails.IsReconciled
...
SELECT        EntryData.EntryDataId as InvoiceNo, EntryData.EntryDataDate as Date, CASE WHEN AliasName IS NOT NULL THEN inventoryitemalias.itemnumber ELSE entrydatadetails.ItemNumber END AS ItemNumber, EntryDataDetails.ItemDescription, 
...
drop table #returns
SELECT        ItemNumber, max(ItemDescription) as ItemDescription , SUM(Quantity*-1)  AS Quantity, AVG(Cost) AS Cost
into #returns
FROM #salesData 
...
drop table #sales
SELECT        ItemNumber,max(ItemDescription) as ItemDescription, SUM(Quantity) AS Quantity, 
                         AVG(Cost) AS Cost
into #sales
FROM   #salesData
WHERE        (Date BETWEEN @startdate AND @endDate)  and quantity > 0  and ItemNumber in (select ItemNumber from #salesData group by itemnumber having sum(Quantity) <> 0)
...
GROUP BY ItemNumber, IsReconciled
...
SELECT        'C#' + AsycudaDocumentRegistrationDate.CNumber + '-' + CAST(xcuda_Item.LineNumber AS varchar(50)) AS PrevDoc, xcuda_HScode.Precision_4, xcuda_Goods_description.Commercial_Description AS Description, 
                         ...
                         isnull(dbo.InventoryItemAlias.ItemNumber,xcuda_HScode.Precision_4) AS ItemNumber, AsycudaDocumentRegistrationDate.AssessmentDate,  AsycudaDocumentRegistrationDate.RegistrationDate, AsycudaDocumentBasicInfo.Reference
into #AsycudaData
...
WHERE        (AsycudaDocumentRegistrationDate.AssessmentDate <= @endDate) AND (xcuda_Item.WarehouseError IS NULL) AND (isnull(AsycudaDocumentBasicInfo.Cancelled, 0) <> 1) AND (isnull(AsycudaDocumentBasicInfo.DoNotAllocate,0) <> 1) 
...
GROUP BY AsycudaDocumentRegistrationDate.DocumentType, xcuda_HScode.Precision_4, xcuda_Goods_description.Commercial_Description, AsycudaDocumentRegistrationDate.CNumber, xcuda_Item.LineNumber, 
                         InventoryItemAlias.AliasName, AsycudaDocumentRegistrationDate.AssessmentDate,  AsycudaDocumentRegistrationDate.RegistrationDate,dbo.InventoryItemAlias.ItemNumber,
						 AsycudaDocumentBasicInfo.Reference

HAVING        (AsycudaDocumentRegistrationDate.DocumentType = 'IM7' OR AsycudaDocumentRegistrationDate.DocumentType = 'OS7')
			   AND (isnull(dbo.InventoryItemAlias.ItemNumber,xcuda_HScode.Precision_4) = xcuda_HScode.Precision_4) 
...
SELECT        Coalesce(OPS.EntryDataId, (case when sales.ItemNumber is null then Null else 'Sales' end),(case when returns.ItemNumber is null then Null else 'Returns' end),(case when adjustments.ItemNumber is null then Null else 'Adjustments' end)) AS InvoiceNo, Coalesce(OPS.ItemNumber, sales.ItemNumber, returns.ItemNumber,adjustments.ItemNumber) AS ItemNumber,  Coalesce(OPS.ItemDescription, sales.ItemDescription, returns.ItemDescription, adjustments.ItemDescription) AS ItemDescription, CAST(Coalesce(OPS.Cost, sales.Cost, returns.cost,adjustments.Cost) AS float) AS Cost, sales.Quantity AS Sales, 
...
FROM            (SELECT        [#Sales].ItemNumber, [#Sales].Quantity, [#Sales].ItemDescription, [#Sales].Cost
						 ...
						 full outer join (SELECT        [#Returns].ItemNumber, [#Returns].Quantity, [#Returns].ItemDescription, [#Returns].Cost
                          FROM            [#Returns]) AS returns on returns.ItemNumber = OPS.ItemNumber
						 full outer join (SELECT        [#adjustments].ItemNumber, [#adjustments].Quantity, [#adjustments].ItemDescription, [#adjustments].Cost
                          FROM            [#adjustments]) AS adjustments on adjustments.ItemNumber = OPS.ItemNumber
GROUP BY sales.Cost, sales.ItemNumber, OPS.ItemNumber, sales.Quantity, OPS.Quantity, OPS.EntryDataId, ops.ItemDescription, sales.ItemDescription, ops.Cost, sales.cost, returns.ItemNumber, returns.ItemDescription, returns.Cost, returns.Quantity,
	 adjustments.ItemNumber, adjustments.ItemDescription, adjustments.Cost, adjustments.Quantity
...
WHERE        (AsycudaDocumentRegistrationDate.AssessmentDate <= @endDate and xcuda_Item.WarehouseError is null) AND (isnull(AsycudaDocumentRegistrationDate.Cancelled,0) <> 1) AND (isnull(AsycudaDocumentRegistrationDate.DoNotAllocate,0) <> 1) 
						--TODO: prevent unregistered entries IM7s
			--and (AsycudaDocumentBasicInfo.CNumber <> null or AsycudaDocumentBasicInfo.IsManuallyAssessed = 1)
...
HAVING        (dbo.AsycudaDocumentRegistrationDate.DocumentType = 'IM7') OR (dbo.AsycudaDocumentRegistrationDate.DocumentType = 'OS7')
...
SELECT        isnull(#OPS.InvoiceNo, 'Asycuda') as InvoiceNo, ISNULL(#OPS.ItemNumber, [#AsycudaData-ItemAlias].ItemNumber) AS ItemNumber, ISNULL(#OPS.ItemDescription, 
...
SELECT        isnull(#OPS.InvoiceNo, 'Asycuda') as InvoiceNo, ISNULL(#OPS.ItemNumber, #AsycudaData.ItemNumber) AS ItemNumber, ISNULL(#OPS.ItemDescription, 
                         #AsycudaData.Description) AS Description, ISNULL(#OPS.QuantityOnHand, #AsycudaData.Amount) AS Quantity,
                         ...
                         ISNULL(#OPS.Cost, 0) AS OPSCost, #AsycudaData.Amount AS [Asycuda-Quantity], #AsycudaData.PiQuantity AS [Asycuda-PiQuantity], ISNULL(#AsycudaData.Cost, 0) AS AsycudaCost, 
                         COALESCE (#OPS.QuantityOnHand, 0) - COALESCE (#AsycudaData.Amount, 0) AS Diff, ISNULL(#OPS.Cost, #AsycudaData.Cost) AS Cost, 
                         (COALESCE (#OPS.QuantityOnHand, 0) - COALESCE (#AsycudaData.Amount, 0)) * ISNULL(#OPS.Cost, #AsycudaData.Cost) AS TotalCost
...
SELECT  distinct     [#OPS].InvoiceNo, [#OPS].ItemNumber,[#OPS].ItemDescription, (case when QuantityOnHand is null then abs([#OPS].Quantity) else [OPS2zero-Adjustment] end)  as Quantity, [#OPS].Cost
into [#OPS to Zero - Overs]
FROM     [#OPS]       left outer join [InventoryItems-NonStock] on [#OPS].ItemNumber = [InventoryItems-NonStock].ItemNumber   
		-- left outer join [#AsycudaSummary] on  [#OPS].ItemNumber = [#AsycudaSummary].ItemNumber              
WHERE      ( [OPS2zero-Adjustment] > 0  ) and [InventoryItems-NonStock].ItemNumber is null --and [#AsycudaSummary].ItemNumber is null
...
SELECT  distinct     [#OPS].InvoiceNo, [#OPS].ItemNumber,[#OPS].ItemDescription, [#OPS].Sales,[#OPS].Returns,[#OPS].QuantityOnHand, isnull([#OPS].Returns,0) - isnull([#OPS].Sales,0) as Quantity, [#OPS].Cost
into [#P2O-Returns]
FROM     [#OPS]   left outer join [InventoryItems-NonStock] on [#OPS].ItemNumber = [InventoryItems-NonStock].ItemNumber     
				                     
WHERE       ((isnull([#OPS].Sales,0) < isnull([#OPS].Returns,0)) or ( ((isnull([#OPS].Sales,0) - isnull([#OPS].Returns,0))) = [#OPS].QuantityOnHand) ) and isnull([#OPS].Returns,0) - isnull([#OPS].Sales,0) > 0 and [InventoryItems-NonStock].ItemNumber is null 
...
SELECT       coalesce([#StockDifferences].InvoiceNo, [#OPS to Zero - Overs].InvoiceNo, [#P2O-Returns].InvoiceNo) as InvoiceNo,
			 ...
			 (isnull(avg([#StockDifferences].[OPS-QuantityOnHand]),0) +  avg(isnull(Adj.Quantity,0)) + avg(isnull([#P2O-Returns].Quantity,0)) + avg(isnull([#OPS to Zero - Overs].Quantity,0))) AS [OPS-Quantity],
			 sum(isnull([Asycuda-Quantity],0)) AS [Asycuda-Quantity],
			 (isnull(avg([#StockDifferences].[OPS-QuantityOnHand]),0) +  avg(isnull(Adj.Quantity,0)) + avg(isnull([#P2O-Returns].Quantity,0)) + avg(isnull([#OPS to Zero - Overs].Quantity,0)) - isnull(sum([Asycuda-Quantity]), 0) ) AS Diff,
			 AVG(isnull([#StockDifferences].Cost,0)) AS Cost,
			 AVG(isnull(TotalCost,0)) AS TotalCost
into #Results
FROM            [#StockDifferences] left outer join [#P2O-Returns] on [#StockDifferences].ItemNumber = [#P2O-Returns].ItemNumber
...
GROUP BY coalesce([#StockDifferences].InvoiceNo, [#OPS to Zero - Overs].InvoiceNo, [#P2O-Returns].InvoiceNo),  coalesce([#StockDifferences].ItemNumber,[#OPS to Zero - Overs].ItemNumber, [#P2O-Returns].ItemNumber)
...
where ((isnull([Asycuda-Quantity], 0) = 0 and isnull([OPS-QuantityOnHand], 0) = 0) --- remain quantities is zero for both asycuda and ops
        and (isnull([Asycuda-PiQuantity], 0)) > isnull([OPS-Quantity],0))--- and pi is less than quantity in question
		and (diff = 0)
...
SELECT  distinct 'P2O-Overs-' + FORMAT(@startdate, 'MMMyy') + '-' + FORMAT(@endDate, 'MMMyy') as [Invoice #], @startdate as [Date],[#Results].InvoiceNo, [#Results].ItemNumber,[#Results].Description, [#Results].[Asycuda-Quantity] as [From Quantity], [#Results].OPSCost, [#Results].[Quantity] as [To Quantity], [#Results].AsycudaCost, abs([#Results].Diff) as Quantity, [#Results].Cost, 'XCD' as Currency
into [#P2O-Overs]
FROM     [#InComplete] as [#Results]       left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber    
...
WHERE        ([#Results].InvoiceNo <> 'Asycuda') and cost <> 0 and [#Results].[Asycuda-Quantity] <> 0
			    ...
			    and [InventoryItems-NonStock].ItemNumber is null
--select '#P2O-Overs'
...
SELECT  distinct  'P2O-Shorts-' + FORMAT(@startdate, 'MMMyy') + '-' + FORMAT(@endDate, 'MMMyy') as [Invoice #], @endDate as [Date], [#Results].InvoiceNo, [#Results].ItemNumber,[#Results].Description, [#Results].[Asycuda-Quantity] as [From Quantity], [#Results].OPSCost, [#Results].[Quantity] as [To Quantity], [#Results].AsycudaCost, [#Results].Diff * -1 as Quantity, [#Results].Cost, 'XCD' as Currency
into [#P2O-Shorts]
FROM     [#InComplete] as [#Results]   left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber   
			
WHERE        ([#Results].InvoiceNo <> 'Asycuda') AND (diff < 0 and [Quantity] >= 0) and [InventoryItems-NonStock].ItemNumber is null 
...
SELECT  distinct      'A2O-Overs-' + FORMAT(@startdate, 'MMMyy') + '-' + FORMAT(@endDate, 'MMMyy') as [Invoice #], @startdate as [Date], [#Results].InvoiceNo, [#Results].ItemNumber,[#Results].Description,[#Results].Returns,[#Results].OPS2Zero,[#Results].Adjustments , [#Results].[Asycuda-Quantity] as [From Quantity], [#Results].OPSCost, [#Results].[Quantity] as [To Quantity], [#Results].AsycudaCost, [#Results].Diff as Quantity, [#Results].Cost, 'XCD' as Currency
into [#A2O-Overs]
FROM     [#InComplete] as [#Results]       left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber                 
WHERE       ([#Results].InvoiceNo <> 'Asycuda' and [#Results].[Asycuda-Quantity] = 0)  and cost <> 0 and (diff > 0) and [InventoryItems-NonStock].ItemNumber is null
			and (Quantity <> [Asycuda-PiQuantity]) -- put this to prevent instructions for closed out items
			and (diff <> [Asycuda-PiQuantity]) -- this where change was already exwarehoused eg - SPY/0799320
...
SELECT  distinct    'A2O-Shorts-' + FORMAT(@startdate, 'MMMyy') + '-' + FORMAT(@endDate, 'MMMyy') as [Invoice #], @endDate as [Date], [#Results].InvoiceNo, [#Results].ItemNumber,[#Results].Description, [#Results].[Asycuda-Quantity] as [From Quantity], [#Results].OPSCost, [#Results].[OPS-Quantity] as [To Quantity], [#Results].AsycudaCost, [#Results].Diff * -1 as Quantity, [#Results].Cost, 'XCD' as Currency
into [#A2O-Shorts]
FROM   [#InComplete] as   [#Results]   left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber                       
WHERE        ([#Results].InvoiceNo = 'Asycuda') AND (diff < 0 and [OPS-Quantity] <= 0) and [InventoryItems-NonStock].ItemNumber is null
...
FROM     [#Results]   left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber                       
WHERE       diff <> 0 and cost <> 0 and  [InventoryItems-NonStock].ItemNumber is null
		and (Quantity <> [Asycuda-PiQuantity]) -- put this to prevent instructions for closed out items
...
FROM     [#Results]   left outer join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber                       
WHERE       Cost = 0 and  [InventoryItems-NonStock].ItemNumber is null and diff <> 0
...
SELECT  distinct     [#Results].InvoiceNo, [#Results].ItemNumber,[#Results].Description, [#Results].[OPS-Quantity], [#Results].OPSCost, [#Results].[Asycuda-Quantity], [#Results].AsycudaCost, [#Results].Diff  as Diff, [#Results].Cost
into [#Non-Stock-Differences]
FROM     [#Results]   inner join [InventoryItems-NonStock] on [#Results].ItemNumber = [InventoryItems-NonStock].ItemNumber                       
...
WHERE        ([#Results].InvoiceNo <> 'Asycuda')  AND (EntryData.EntryDataDate BETWEEN @startDate AND @enddate) and AsycudaSalesAllocations.Status is not null
...
select [#InComplete].* from [#InComplete] left outer join [#P2O-Overs] on [#InComplete].ItemNumber = [#P2O-Overs].ItemNumber
							left outer join [#P2O-Shorts] on [#InComplete].ItemNumber = [#P2O-Shorts].ItemNumber
							left outer join [#A2O-Overs] on [#InComplete].ItemNumber = [#A2O-Overs].ItemNumber
							left outer join [#A2O-Shorts] on [#InComplete].ItemNumber = [#A2O-Shorts].ItemNumber
							left outer join [#Non-Stock-Differences] on [#InComplete].ItemNumber = [#Non-Stock-Differences].ItemNumber
...
FROM    AdjustmentDetails LEFT OUTER JOIN
                 InventoryItemAlias ON AdjustmentDetails.ItemNumber = InventoryItemAlias.AliasName LEFT OUTER JOIN
                 AsycudaDocumentItemEntryDataDetails ON AdjustmentDetails.EntryDataDetailsId = AsycudaDocumentItemEntryDataDetails.EntryDataDetailsId
WHERE (AdjustmentDetails.EffectiveDate BETWEEN @startdate AND @endDate) AND (AsycudaDocumentItemEntryDataDetails.EntryDataDetailsId IS NULL and isnull(AdjustmentDetails.IsReconciled, 0) <> 1)
GROUP BY CASE WHEN AliasName IS NOT NULL THEN inventoryitemalias.itemnumber ELSE AdjustmentDetails.ItemNumber END, AdjustmentDetails.ItemDescription, AdjustmentDetails.EntryDataId, 
...
declare @ItemNumber varchar(50) = '7IT/PR-LBL-2X1'
select 'Adjustments Data'
select * from  #adjmentsData where itemnumber = @ItemNumber
select 'Sales Data'
select * from  #salesData where itemnumber = @ItemNumber
...
FROM    AdjustmentShortAllocations LEFT OUTER JOIN
                 AsycudaItemPiQuantityData ON AdjustmentShortAllocations.PreviousItem_Id = AsycudaItemPiQuantityData.Item_Id
WHERE ((AsycudaItemPiQuantityData.Item_Id IS NULL)) --AdjustmentShortAllocations.ItemNumber = 'AWL/G8044QT' AND 
