...
namespace WaterNut.Business.Services.Custom_Services.DataModels.Custom_DataModels.CreatingEx9
{
    ...
    {
        private static bool isDBMem = true;
        ...
        {
            return isDBMem
                ? await GetEx9DataByDateRange.Execute(dateFilter).ConfigureAwait(false)
                : await new GetEx9DataByDateRangeMem().Execute(dateFilter).ConfigureAwait(false);
        }
