...
namespace WaterNut.Business.Services.Custom_Services.DataModels.Custom_DataModels.CreatingEx9
{
    ...
    public partial class CreateEx9Mem : ICreateEx9
    {
        ...
        public  List<PiSummary> DocSetPi = new List<PiSummary>();
            ...
            bool universalPIcheck, bool itemPIcheck)
        {
            try
            {
             ...
             var realStartDate = GetWholeRangeFilter(filterExp, out var realEndDate,  out var exPro, out var rdateFilter, out var realFilterExp);
             var startDate = realStartDate;
             var getEx9DataMem = PrepareEx9MemData(realFilterExp,rdateFilter);
             ...
             while (startDate < realEndDate)
             {
                 ...
                 var allocationDataBlocks =
                     (await CreateAllocationDataBlocks(currentFilter + exPro, errors, dateFilter, getEx9DataMem).ConfigureAwait(false))
                     .Where(x => x.Allocations.Count > 0).ToList();

                if (allocationDataBlocks == null || !allocationDataBlocks.ToList().Any()) continue;
                 ...
                 foreach (var dfp in dutylst)
                 {
                     ...
                     await CreateDutyFreePaidEntries(docSet, documentType, ex9BucketType, isGrouped, checkQtyAllocatedGreaterThanPiQuantity, checkForMultipleMonths, applyEx9Bucket, applyHistoricChecks, applyCurrentChecks, perInvoice, autoAssess, overPIcheck, universalPIcheck, itemPIcheck, allocationDataBlocks, dfp, startDate, endDate, docs);
                 }
        ...
        private async Task CreateDutyFreePaidEntries(AsycudaDocumentSet docSet, string documentType, string ex9BucketType,
            ...
            bool applyHistoricChecks, bool applyCurrentChecks, bool perInvoice, bool autoAssess, bool overPIcheck,
            bool universalPIcheck, bool itemPIcheck, List<AllocationDataBlock> allocationDataBlocks, string dfp, DateTime startDate, DateTime endDate, List<DocumentCT> docs)
        {
            ...
            foreach (var entryType in entryTypes)
            {
                ...
                var itemSalesPiSummarylst = GetItemSalesPiSummary(docSet.ApplicationSettingsId, startDate, endDate, dfp, entryType.Key);
        ...
        {
            return DataSpace.BaseDataModel.Instance.Customs_Procedures
        ...
        private  string CreateDateRangeFilter(DateTime realStartDate, DateTime realEndDate)
        {
            return $@"InvoiceDate >= ""{realStartDate:MM/dd/yyyy}"" && InvoiceDate <= ""{realEndDate.AddHours(23):MM/dd/yyyy HH:mm:ss}""";
        }

        private  string GetDataFilters()
        {
            return " && (PreviousDocumentItem != null && PreviousDocumentItem.AsycudaDocument.Customs_Procedure.CustomsOperations.Name == \"Warehouse\" )" +
        ...
        private GetEx9DataMem PrepareEx9MemData(string filterExp, string rdateFilter)
        {
           return new GetEx9DataMem(filterExp,rdateFilter);
        }
            ...
            DateTime endDate, string dfp, string entryDataType)
        {
            try
            {
                    ...
                    return GetPiSummary(applicationSettingsId, startDate, endDate, dfp, entryDataType);
        ...
        public bool freashStart = true;
        private  List<ItemSalesPiSummary> GetPiSummary(int applicationSettingsId, DateTime startDate,
            DateTime endDate, string dfp, string entryType)
        {
            try
            {
                ...
                SummaryInititalization(applicationSettingsId, entryType, startDate, endDate);
        ...
        private void SummaryInititalization(int applicationSettingsId,  string entryType,
            DateTime startDate, DateTime endDate)
        {
            InitializeUniversalData(applicationSettingsId);
        ...
        private void AttachSupplier(DocumentCT cdoc)
        {
            ...
            if (firstItm == null) return;
                ...
                var exporter = GetExporters().FirstOrDefault(x => x.Key.CNumber == firstItm.xcuda_PreviousItem.Prev_reg_nbr).Value;
                if(exporter != null)
                {
        ...
        Dictionary<(int PreviousItemId, string CNumber), xcuda_Exporter> _exporters = null;
        private Dictionary<(int PreviousItemId, string CNumber), xcuda_Exporter> GetExporters()
        {
            if(_exporters != null) return _exporters;
            _exporters = new DocumentDSContext().xcuda_Exporter.Select(x => new { PreviousItemId = x.Traders_Id, CNumber = x.xcuda_Traders.xcuda_ASYCUDA.xcuda_Identification.xcuda_Registration.Number, value = x}).ToDictionary(x => (x.PreviousItemId,CNumber: x.CNumber), x => x.value);
            return _exporters;
        }
        ...
        private async Task<IEnumerable<AllocationDataBlock>> CreateAllocationDataBlocks(string filterExpression,
            List<string> errors,
            string dateFilter, GetEx9DataMem getEx9DataMem)
        {
            try
            {
                StatusModel.Timer("Getting ExBond Data");
                var ex9Data = await getEx9DataMem.Execute(filterExpression, dateFilter).ConfigureAwait(false);
        ...
        private DataSpace.BaseDataModel.MyPodData CreateMyPodData(List<EX9Allocations> s)
        {
            try
            {
        ...
        {
            foreach (var a in allocations) a.xStatus = xstatus;
                        ...
                        for (var s = currentSalesItemIndex; s < salesLst.Count(); s++)
                        {
                            ...
                            for (var i = currentAllocationItemIndex; i < saleAllocationsLst.Count(); i++)
                            {
                                ...
                                else
                                {
                                    if (piData > 0)
                                    {
                                        UpdatePIData(piData, salesFactor, allocation);
                                    }
                                }
        ...
        private  void UpdatePIData(double piData, double salesFactor, AsycudaSalesAllocations allocation)
        {
