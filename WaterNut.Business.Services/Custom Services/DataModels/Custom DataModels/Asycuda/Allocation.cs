    ...
    public partial class AllocationsBaseModel
    {
        ...
        private async Task AllocateSalesByMatchingSalestoAsycudaEntriesOnItemNumber(bool allocateToLastAdjustment)
        {
            ...
            Parallel.ForEach(itemSetsValues.OrderBy(x => x.Key)
                                    ...
                                    // .Where(x => x.Key.Contains("CLS/8536")) // 
            ...
            //{
                try
                {
                   ...
                   // Debug.WriteLine($"Processing {itm.Key} - {t} with {itm.SalesList.Count} Sales: {0} of {itm.SalesList.Count}");
                    //StatusModel.Refresh();
                var sales = itm.SalesList.OrderBy(x => x.Sales.EntryDataDate).ThenBy(x => x.EntryDataId).ThenBy(x => x.EntryDataDetailsId).ThenByDescending(x => x.Quantity)/**/.ToList();
            ...
            using (var ctx = new xcuda_ItemService())
            {
                var lst = await ctx.Getxcuda_ItemByExpressionNav(
                    ...
                    new Dictionary<string, string>() { { "AsycudaDocument", (BaseDataModel.Instance.CurrentApplicationSettings.OpeningStockDate.HasValue ? $"AssessmentDate >= \"{BaseDataModel.Instance.CurrentApplicationSettings.OpeningStockDate}\" && "
                                                                                : "") + "(CNumber != null || IsManuallyAssessed == true) && (Extended_customs_procedure == \"7000\" ||Extended_customs_procedure == \"7400\" ||Extended_customs_procedure == \"7100\" || Extended_customs_procedure == \"9000\") && DoNotAllocate != true" } }
