...
namespace WaterNut.Business.Services.Custom_Services.DataModels.Custom_DataModels.Asycuda.OldSalesAllocator
{
    public class OldSalesAllocator
    {
        ...
        public List<List<KeyValuePair<int, (List<ExistingAllocations> ExistingAllocations, List<EntryDataDetails>
                EntryDataDetails, List<xcuda_Item> XcudaItems, List<AsycudaSalesAllocations> dbAllocations)>>>
            GetAllocations(bool allocateToLastAdjustment,
                ConcurrentDictionary<(DateTime EntryDataDate, string EntryDataId, string ItemNumber, int InventoryItemId
                    ),
                    AllocationsBaseModel.ItemSet> itemSets, ConcurrentDictionary<int, xcuda_Item> xcudaItems)
        {
            _asycudaItems = xcudaItems;
            ...
            var itemGroups = DataSpace.Utils.CreateItemSet(rawSet);


            var ggitms = GroupOfGroups(itemGroups);

            var alloLst = ParallelEnumerable.Select<KeyValuePair<int, (List<int> Lst, KeyValuePair<int, List<((string ItemNumber, int InventoryItemId) Key, List<dynamic> Value)>> group)>, List<KeyValuePair<int, (List<ExistingAllocations> ExistingAllocations, List<EntryDataDetails> EntryDataDetails, List<xcuda_Item> XcudaItems, List<AsycudaSalesAllocations> dbAllocations)>>>(ParallelEnumerable.WithDegreeOfParallelism<KeyValuePair<int, (List<int> Lst, KeyValuePair<int, List<((string ItemNumber, int InventoryItemId) Key, List<dynamic> Value)>> group)>>(ggitms
                    .AsParallel(), 1 /*Convert.ToInt32(Environment.ProcessorCount * BaseDataModel.Instance.ResourcePercentage)*/), x => AllocateSales(allocateToLastAdjustment, x))
                .ToList();
            return alloLst;
        }
        ...
        {
            var res = ParallelEnumerable.Select<List<AllocationsBaseModel.ItemSet>, List<(List<EntryDataDetails> Sales, List<xcuda_Item> asycudaItems)>>(groupItemSets.Value.group.Value.Select(v => v.Value.Select(z => ((KeyValuePair<(DateTime EntryDataDate, string EntryDataId, string ItemNumber, int InventoryItemId), AllocationsBaseModel.ItemSet >)z).Value).ToList())
                    .AsParallel()
                    .WithDegreeOfParallelism(Convert.ToInt32(Environment.ProcessorCount *
                                                             BaseDataModel.Instance.ResourcePercentage)), x => AllocateSales(allocateToLastAdjustment, x))
                .ToList();
            ...
            Dictionary<int, List<((string ItemNumber, int InventoryItemId) Key, List<dynamic> Value)>> itemGroups)
        {
            ...
            var workingitemGroups = itemGroups.Select(parentgroup => (Lst: parentgroup.Value.SelectMany(x =>
                    x.Value.SelectMany(z => ((AllocationsBaseModel.ItemSet)z.Value).EntriesList.Select(a => a.Item_Id).ToList()).Distinct()
                        .ToList()).ToList(),
        ...
        private List<(List<EntryDataDetails> Sales, List<xcuda_Item> asycudaItems)> AllocateSales(bool allocateToLastAdjustment, List<AllocationsBaseModel.ItemSet> itemSets)
        {
        ...
        private static List<xcuda_Item> SortAsycudaItems(AllocationsBaseModel.ItemSet itm)
        {
        ...
        private static List<EntryDataDetails> SortEntryDataDetailsList(AllocationsBaseModel.ItemSet itm)
        {
        ...
        public string CheckExistingStock(string itemNumber, DateTime salesEntryDataDate)
        {
