        ...
        public async Task Execute(ApplicationSettings applicationSettings, bool allocateToLastAdjustment, string lst= null)
        {
            try
            {
                ...
                itemSets
                    .AsParallel()
                    .WithDegreeOfParallelism(Convert.ToInt32(Environment.ProcessorCount * DataSpace.BaseDataModel.Instance.ResourcePercentage))
                    .ForAll(x => new ReallocateExistingEx9().Execute(x).Wait())
                    ; // to prevent changing allocations when im7 info changes
                ...
                itemSets
                    ...
                    .ForAll(x => new OldSalesAllocator().AllocateSalesByMatchingSalestoAsycudaEntriesOnItemNumber(allocateToLastAdjustment, x).Wait());
