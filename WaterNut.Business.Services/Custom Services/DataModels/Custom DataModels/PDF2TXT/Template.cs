...
//        public Template(Invoices ocrInvoices)
//        {
...
//        {
//            Console.WriteLine($"[OCR DEBUG] Template.Read: Starting read for Template ID {OcrInvoices.Id}, Name '{OcrInvoices.Name}'. Received {text.Count} lines.");
//            try
//            {
...
//            {
//                Console.WriteLine($"[OCR DEBUG] Template.Read: No instances found for Template ID {OcrInvoices.Id}. Returning empty list structure.");
//                // Return the expected structure but with an empty inner list
//                return new List<dynamic> { new List<IDictionary<string, object>>() };
//            }

//            Console.WriteLine($"[OCR DEBUG] Template.Read: Finished read for Template ID {OcrInvoices.Id}. Returning {finalResultList.Count} assembled instances.");
...
//        {
//            Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Processing Part ID {part.OCR_Part.Id}" + (filterInstance.HasValue ? $" for Filter Instance {filterInstance.Value}" : " (Top Level)"));
...
//                     {
//                         Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: No direct data for Filter Instance {filterInstance.Value}, but children might. Processing instance {filterInstance.Value} for child aggregation.");
...
//                 {
//                     Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: No relevant instances found" + (filterInstance.HasValue ? $" for Filter Instance {filterInstance.Value}" : "") + ". Returning empty list.");
//                     return finalPartItems;
//                 }

//                Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: Processing instances [{string.Join(", ", instancesToProcess)}]" + (filterInstance.HasValue ? $" for Filter Instance {filterInstance.Value}" : ""));
...
//                {
//                    Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: Processing instance {currentInstance}.");
...
//                        Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}, Instance {currentInstance}: Child Part ID {childPartId} - Raw items: {rawChildItems.Count}, Deduplicated items: {deduplicatedChildItems.Count} (using positional index).");
...
//                        Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}, Instance {currentInstance}: Attaching {deduplicatedChildItems.Count} deduplicated child items for Child Part ID {childPartId} under field '{fieldname}'.");
...
//                            } else {
//                                Console.WriteLine($"[OCR WARNING] Template.SetPartLineValues: Field '{fieldname}' exists but is not a list for child {childPartId}. Overwriting.");
...
//                    {
//                         Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: Adding assembled item for instance {currentInstance} to results.");
//                         finalPartItems.Add(parentItem);
//                    } else {
//                         Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Part ID {part.OCR_Part.Id}: Skipping empty or incomplete item for instance {currentInstance}.");
//                    }
//                }


//                Console.WriteLine($"[OCR DEBUG] Template.SetPartLineValues: Finished processing Part ID {part.OCR_Part.Id}. Returning {finalPartItems.Count} items.");
//                return finalPartItems;
//            }
//            catch (Exception e)
//            {
//                Console.WriteLine($"[OCR ERROR] Template.SetPartLineValues: Exception processing Part ID {part.OCR_Part.Id}: {e}");
//                throw;
//            }
//        }
