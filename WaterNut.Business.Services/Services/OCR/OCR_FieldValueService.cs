...
namespace OCR.Business.Services
{
   ...
   [ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall,
    ...
    public partial class OCR_FieldValueService : IOCR_FieldValueService, IDisposable
    {
        //private readonly OCRContext dbContext;

        public bool StartTracking { get; set; }

        public OCR_FieldValueService()
        {
            try
            {
            ...
            //using (var scope = new TransactionScope(TransactionScopeOption.Required,
               ...
               // {
                  using ( var dbContext = new OCRContext(){StartTracking = StartTracking})
                  {
                    ...
                    IEnumerable<OCR_FieldValue> entities = await set.AsNoTracking().ToListAsync()
        ...
        public async Task<OCR_FieldValue> GetOCR_FieldValueByKey(string Id, List<string> includesLst = null, bool tracking = true)
        {
            try
            {
              ...
              using ( var dbContext = new OCRContext(){StartTracking = StartTracking})
              {
				...
				var set = AddIncludes(includesLst, dbContext);
                OCR_FieldValue entity = await set.AsNoTracking().SingleOrDefaultAsync(x => x.Id == i).ConfigureAwait(continueOnCapturedContext: false);
                    ...
                    foreach (var itm in navExp)
                    {
                        switch (itm.Key)
                        {
                            case "Field":
                                return
                                    await
                                        GetWhere<Fields>(dbContext, exp, itm.Value, "FieldValue", "SelectMany", includesLst)
										.ConfigureAwait(continueOnCapturedContext: false);
            ...
            int totalrow, List<string> includesLst = null, bool tracking = true)
        {
            try
            {
                ...
                var res = new ConcurrentQueue<List<OCR_FieldValue>>();
                    ...
                    //  for (int bat = 0; bat < batches; bat++)
                    {
                        try
                        {
                            using (var dbContext = new OCRContext(){StartTracking = StartTracking})
                            {
                                ...
                                var set = AddIncludes(includesLst, dbContext);
                                IQueryable<OCR_FieldValue> dset;
                                if (exp == "All")
                                {
        ...
        public async Task<bool> DeleteOCR_FieldValue(string Id)
        {
            try
            {
              ...
              {
                var i = Convert.ToInt32(Id);
                OCR_FieldValue entity = await dbContext.OCR_FieldValue
        ...
        public async Task<bool> RemoveSelectedOCR_FieldValue(IEnumerable<string> lst)
        {
            try
            {
                StatusModel.StartStatusUpdate("Removing OCR_FieldValue", lst.Count());
                var t = Task.Run(() =>
                {
                    using (var ctx = new OCR_FieldValueService())
                    {
                        foreach (var item in lst.ToList())
                        {

                            ctx.DeleteOCR_FieldValue(item).Wait();
		...
		public async Task<int> Count(string exp)
        {
            try
            {
                using (OCRContext dbContext = new OCRContext(){StartTracking = StartTracking})
                {
                    ...
                    if (exp == "All")
                    {
                        return await dbContext.OCR_FieldValue
                                    .AsNoTracking()
									.CountAsync()
									.ConfigureAwait(continueOnCapturedContext: false);
                    }
            ...
            try
            {
                ...
                using (var dbContext = new OCRContext(){StartTracking = StartTracking})
                {
                    ...
                    foreach (var itm in navExp)
                    {
                        ...
                        {
                            case "Field":
                                return await CountWhere<Fields>(dbContext, exp, itm.Value, "FieldValue", "SelectMany")
											.ConfigureAwait(continueOnCapturedContext: false);
						}
                    }
                    return await dbContext.OCR_FieldValue.Where(exp == "All" || exp == null ? "Id != null" : exp)
											.AsNoTracking()
                                            .CountAsync()
											.ConfigureAwait(continueOnCapturedContext: false);
                }
                                                                                 ...
                                                                                 Dictionary<string, string> navExp, IEnumerable<string> includeLst = null)
        {
            try
            {
                ...
                {
                    dbContext.Database.CommandTimeout = 0;
                    if ((string.IsNullOrEmpty(exp) && navExp.Count == 0) || exp == "None") return new List<OCR_FieldValue>();
                    ...
                    foreach (var itm in navExp)
                    {
                        switch (itm.Key)
                        {
                            case "Field":
                                return
                                    await
                                        LoadRangeWhere<Fields>(startIndex, count, dbContext, exp, itm.Value, "FieldValue", "SelectMany")
													.ConfigureAwait(continueOnCapturedContext: false);
                    ...
                    return await set//dbContext.OCR_FieldValue
								.AsNoTracking()
                                .Where(exp == "All" || exp == null ? "Id != null" : exp)
								.OrderBy(y => y.Id)
		...
		private static async Task<IEnumerable<OCR_FieldValue>> LoadRangeWhere<T>(int startIndex, int count,
        ...
        private static async Task<IEnumerable<OCR_FieldValue>> GetWhere<T>(OCRContext dbContext,
             ...
             try
             {
                 using (var dbContext = new OCRContext(){StartTracking = StartTracking})
                 {
					...
					decimal res = 0;
                     ...
                     if (whereExp == "All")
                     {
                          res = Convert.ToDecimal(dbContext.OCR_FieldValue.AsNoTracking().Sum(field));
                     }
                     else
                     {
                         res = Convert.ToDecimal(dbContext.OCR_FieldValue.AsNoTracking().Where(whereExp).Sum(field));
                     }
        ...
        public async Task<decimal> SumNav( string exp, Dictionary<string, string> navExp, string field)
        {
            try
            {
                ...
                {
                    dbContext.Database.CommandTimeout = 0;
                    if (!dbContext.OCR_FieldValue.Any()) return 0;
                    if (exp == "All" && navExp.Count == 0)
                    {
                        return Convert.ToDecimal(dbContext.OCR_FieldValue
										.AsNoTracking()
                                        .Sum(field)??0);
                    }
                    foreach (var itm in navExp)
                    {
                        ...
                        {
                            case "Field":
                                return await SumWhere<Fields>(dbContext, exp, itm.Value, "FieldValue", field, "SelectMany")
											.ConfigureAwait(continueOnCapturedContext: false);
						}
                    }
                    return Convert.ToDecimal(dbContext.OCR_FieldValue.Where(exp == "All" || exp == null ? "Id != null" : exp)
											.AsNoTracking()
                                            .Sum(field)??0);
                }
             ...
             try
             {
                 using (var dbContext = new OCRContext(){StartTracking = StartTracking})
                 {
					...
					string res = "";
                     ...
                     if (whereExp == "All")
                     {
                          res = Convert.ToString(dbContext.OCR_FieldValue.AsNoTracking().Min(field));
                     }
                     else
                     {
                         res = Convert.ToString(dbContext.OCR_FieldValue.AsNoTracking().Where(whereExp).Min(field));
                     }
		...
		private static IQueryable<OCR_FieldValue> AddIncludes(IEnumerable<string> includesLst, OCRContext dbContext)
       {
		 try
			{
			   if (includesLst == null) includesLst = new List<string>();
			   var set =(DbQuery<OCR_FieldValue>) dbContext.OCR_FieldValue; 
	   ...
	   private IQueryable<OCR_FieldValue> AddWheres(List<string> expLst, IQueryable<OCR_FieldValue> set)
        {
            try
            {
