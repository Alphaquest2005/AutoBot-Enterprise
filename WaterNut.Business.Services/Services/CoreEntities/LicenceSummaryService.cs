...
namespace CoreEntities.Business.Services
{
   ...
   [ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall,
    ...
    public partial class LicenceSummaryService : ILicenceSummaryService, IDisposable
    {
        ...
        public async Task<bool> RemoveSelectedLicenceSummary(IEnumerable<string> lst)
        {
            try
            {
                StatusModel.StartStatusUpdate("Removing LicenceSummary", lst.Count());
                var t = Task.Run(() =>
                {
                    using (var ctx = new LicenceSummaryService())
                    {
                        foreach (var item in lst.ToList())
                        {

                            ctx.DeleteLicenceSummary(item).Wait();
		...
		public async Task<int> CountNav(string exp, Dictionary<string, string> navExp)
        {
            try
            {
                ...
                using (var dbContext = new CoreEntitiesContext(){StartTracking = StartTracking})
                {
                    ...
                    return dbContext.LicenceSummary.Where(exp == "All" || exp == null ? "RowNumber != null" : exp)
											.AsNoTracking()
                                            .Count();
                }
                                                                                 ...
                                                                                 Dictionary<string, string> navExp, IEnumerable<string> includeLst = null)
        {
            try
            {
                using (var dbContext = new CoreEntitiesContext(){StartTracking = StartTracking})
                {
                    ...
                    return set//dbContext.LicenceSummary
								.AsNoTracking()
                                .Where(exp == "All" || exp == null ? "RowNumber != null" : exp)
								.OrderBy(y => y.RowNumber)
			        ...
			        public Task<IEnumerable<LicenceSummary>> GetLicenceSummaryByAsycudaDocumentSetId(string AsycudaDocumentSetId, List<string> includesLst = null)
        {
            try
            {
                using ( var dbContext = new CoreEntitiesContext(){StartTracking = StartTracking})
              {
                ...
                var set = AddIncludes(includesLst, dbContext);
                IEnumerable<LicenceSummary> entities = set//dbContext.LicenceSummary
                                      .AsNoTracking()
        ...
        public async Task<decimal> SumNav( string exp, Dictionary<string, string> navExp, string field)
        {
            try
            {
                ...
                {
                    dbContext.Database.CommandTimeout = 0;
                    if (!dbContext.LicenceSummary.Any()) return 0;
                    if (exp == "All" && navExp.Count == 0)
                    {
