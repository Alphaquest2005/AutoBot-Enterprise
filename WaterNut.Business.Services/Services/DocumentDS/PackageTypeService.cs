        ...
        public async Task<PackageType> GetPackageTypeByKey(string PackageType1, List<string> includesLst = null, bool tracking = true)
        {
            try
            {
              ...
              using ( var dbContext = new DocumentDSContext(){StartTracking = StartTracking})
              {
                var i = PackageType1;
				var set = AddIncludes(includesLst, dbContext);
                PackageType entity = await set.AsNoTracking().SingleOrDefaultAsync(x => x.PackageType1 == i).ConfigureAwait(continueOnCapturedContext: false);
            ...
            try
            {
              using ( var dbContext = new DocumentDSContext(){StartTracking = StartTracking})
              {
                ...
                PackageType entity = await dbContext.PackageTypes
													.SingleOrDefaultAsync(x => x.PackageType1 == i)
													.ConfigureAwait(continueOnCapturedContext: false);
                if (entity == null)
                    return false;
		...
		public async Task<int> CountNav(string exp, Dictionary<string, string> navExp)
        {
            try
            {
                ...
                using (var dbContext = new DocumentDSContext(){StartTracking = StartTracking})
                {
                    ...
                    return await dbContext.PackageTypes.Where(exp == "All" || exp == null ? "PackageType1 != null" : exp)
											.AsNoTracking()
                                            .CountAsync()
											.ConfigureAwait(continueOnCapturedContext: false);
                }
