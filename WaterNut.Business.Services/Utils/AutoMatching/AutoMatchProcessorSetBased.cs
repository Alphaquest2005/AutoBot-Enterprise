...
{
    public class AutoMatchSetBasedProcessor
    {
        ...
        public async Task AutoMatch(int applicationSettingsId, bool overwriteExisting, List<List<(string ItemNumber, int InventoryItemId)>> itemSets)
        {
            try
            {
                ...
                var rawData =
                    GetAllDiscrepancyDetails(overwriteExisting, itemSets.SelectMany(x => x.ToList()).ToList());
                             //.Where(x => x.EntryDataId == "Asycuda-C#33687-24").ToList();

                if (!rawData.Any()) return;

                ClearDocSetAllocations(rawData, itemSets);
        ...
        public void ClearDocSetAllocations(List<AdjustmentDetail> rawData, List<List<(string ItemNumber, int InventoryItemId)>> itemSets)
        {
