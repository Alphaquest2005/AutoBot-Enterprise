// <autogenerated>
//   This file was generated by T4 code generator AllClientModels.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>


using AdjustmentQS.Client.DTO;
using TrackableEntities.Client;



namespace AdjustmentQS.Client.Entities
{

    public partial class TODO_PreDiscrepancyErrors 
    {
       public TODO_PreDiscrepancyErrors()
        {
            this.DTO = new DTO.TODO_PreDiscrepancyErrors();//{TrackingState = TrackableEntities.TrackingState.Added}
            _changeTracker = new ChangeTrackingCollection<DTO.TODO_PreDiscrepancyErrors>(this.DTO);

            CustomClassStartUp();
            MyNavPropStartUp();
            IIdentifiableEntityStartUp();
            AutoGenStartUp();
        }
    partial void CustomClassStartUp();
    partial void AutoGenStartUp();
    partial void MyNavPropStartUp();
    partial void IIdentifiableEntityStartUp();
   
    }
}
		