// <autogenerated>
//   This file was generated by T4 code generator AllClientModels.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>


using InventoryQS.Client.DTO;
using TrackableEntities.Client;



namespace InventoryQS.Client.Entities
{

    public partial class TariffCategoryCodeSuppUnit 
    {
       public TariffCategoryCodeSuppUnit()
        {
            this.DTO = new DTO.TariffCategoryCodeSuppUnit();//{TrackingState = TrackableEntities.TrackingState.Added}
            _changeTracker = new ChangeTrackingCollection<DTO.TariffCategoryCodeSuppUnit>(this.DTO);

            CustomClassStartUp();
            MyNavPropStartUp();
            IIdentifiableEntityStartUp();
            AutoGenStartUp();
        }
    partial void CustomClassStartUp();
    partial void AutoGenStartUp();
    partial void MyNavPropStartUp();
    partial void IIdentifiableEntityStartUp();
   
    }
}
		