    ...
    public class ClientService<T> : ClientBase<T>, IDisposable where T : class
    {
        public ClientService()
            : base( //new NetTcpBinding(SecurityMode., true) 
                ...
                new NetTcpBinding(SecurityMode.None, true)
                {
                    ...
                    ReliableSession =
                        new OptionalReliableSession(new ReliableSessionBindingElement(false)) {Enabled = false},
                ...
                new EndpointAddress("net.tcp://localhost:8734/" +
                                    typeof(T).FullName.Replace(".Client.Contracts.I", ".Business.Services."))
            )
        {
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
                try
                {
                    if (State != CommunicationState.Faulted) Close();
                }
                finally
                {
                    if (State != CommunicationState.Closed) Abort();
