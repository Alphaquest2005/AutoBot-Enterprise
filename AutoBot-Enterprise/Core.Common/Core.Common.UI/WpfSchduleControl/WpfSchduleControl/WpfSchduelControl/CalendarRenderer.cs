...
namespace WpfSchduleControl
{
    ...
    public class CalendarRenderer
    {
        ...
        /// <summary>
        /// Possible corners to pass to the <see cref="RoundRect"/> method
        /// </summary>
        [Flags()]
        public enum Corners
        {
        ...
        /// <returns></returns>
        public static Path RoundRect(Rect r, int radius)
        {
            return RoundRect(r, radius, Corners.All);
        }

        /// <summary>
        /// Creates a Rect with the specified corners rounded
        /// </summary>
        /// <param name="r"></param>
        /// <param name="radius"></param>
        /// <param name="corners"></param>
        /// <returns></returns>
        public static Path RoundRect(Rect r, int radius, Corners corners)
        {
            ...
            geo.AddGeometry(new LineGeometry(new Point(r.Left + nw, r.Top), new Point(r.Right - ne, r.Top)));

            if (ne > 0)
            {
                .AddArc(Rect.FromLTRB(r.Right - ne, r.Top, r.Right, r.Top + ne),
                    -90, 90);
            }
            ...
            if (se > 0)
            {
                path.AddArc(Rect.FromLTRB(r.Right - se, r.Bottom - se, r.Right, r.Bottom),
                    0, 90);
            }
            ...
            if (sw > 0)
            {
                path.AddArc(Rect.FromLTRB(r.Left, r.Bottom - sw, r.Left + sw, r.Bottom),
                    90, 90);
            }
            ...
            if (nw > 0)
            {
                path.AddArc(Rect.FromLTRB(r.Left, r.Top, r.Left + nw, r.Top + nw),
                    180, 90);
            }
        ...
        private Rect[] _dayNameHeaderColumns;
        ...
        /// </summary>
        public Rect[] DayNameHeaderColumns
        {
        ...
        /// <summary>
        /// Creates a Rect with item roundess
        /// </summary>
        /// <param name="bounds"></param>
        /// <returns></returns>
        public Path ItemRect(CalendarRendererItemBoundsEventArgs evtData, Rect bounds)
        {
                ...
                ! evtData.Item.IsOnDayTop && evtData.Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                ...
                */
                ...
                path.AddArc(new Rect(d.X - sq, d.Y, sq, sq), -90, 90);
                path.AddLine(new Point(d.X, d.Y + sq), new Point(d.X, e.Y - sq));
                path.AddArc(new Rect(e.X - sq, e.Y - sq, sq, sq), 0, 90);
            ...
            else
            {
                ...
                return RoundRect(bounds, ItemRoundness, crns);
            }
        }

        /// <summary>
        /// Fills the specified Rect with item border roundness
        /// </summary>
        /// <param name="g"></param>
        /// <param name="bounds"></param>
        ...
        /// <returns></returns>
        public void ItemFill(CalendarRendererItemBoundsEventArgs e, Rect bounds, Color north, Color south)
        {
            ...
            using (Path r = ItemRect(e, bounds))
            {
                using (LinearGradientBrush b = new LinearGradientBrush(bounds, north, south, 90))
                {
                    e.DrawingContext.FillPath(b, r);
                }
            }
        }

        public void ItemPattern(CalendarRendererItemBoundsEventArgs e, Rect bounds, Color patternColor)
        {
            if (bounds.Width <= 0 || bounds.Height <= 0)
            {
                return;
            ...
            using (Path r = ItemRect(e, bounds))
            {
                using (Brush b = new HatchBrush(e.Item.Pattern, patternColor, Colors.Transparent))
                {
                    e.DrawingContext.FillPath(b, r);
                }
            }
        ...
        /// <summary>
        /// Draws the specified Rect with item border roundnesss
        /// </summary>
        /// <param name="g"></param>
        /// <param name="bounds"></param>
        ...
        /// <returns></returns>
        public void ItemBorder(CalendarRendererItemBoundsEventArgs e, Rect bounds, Color color, float width)
        {
            using (Path r = ItemRect(e, bounds))
            {
                using (Pen p = new Pen(color, width))
                {
                    e.DrawingContext.DrawPath(p, r);
                }
            }
        }
        ...
        public void PerformLayout(bool performItemsLayout)
        {
            ...
            TimeScaleBounds = Rect.Empty;

            if (Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                #region Expanded days
                TimeScaleBounds = new Rect(0, 0, TimeScaleWidth, Calendar.ClientRect.Height);
                ...
                for (int i = 0; i < Calendar.Days.Length; i++)
                {
                    ...
                    day.SetBounds(new Rect(curLeft, curTop, dayWidth, dayHeight));
                    day.DayTop.SetBounds(new Rect(curLeft, day.HeaderBounds.Bottom, dayWidth, DayTopHeight));
                    curLeft += dayWidth + 1;
                    //int k = 0;
                    ...
                    for (int j = 0; j < day.TimeUnits.Length; j++)
                    {
                        ...
                        else
                        {
                            ...
                            //unit.SetBounds(new Rect(day.Bounds.Left, day.BodyBounds.Top + k++ * TimeScaleUnitHeight, day.Bounds.Width, TimeScaleUnitHeight));
                        }
                        unit.SetBounds(new Rect(day.Bounds.Left, utop, day.Bounds.Width, TimeScaleUnitHeight));
            ...
            else
            {
                ...
                _dayNameHeaderColumns = new Rect[7];
                int j = 0;

                for (int i = 0; i < Calendar.Days.Length; i++)
                {
                    Calendar.Days[i].SetBounds(new Rect(curLeft, curTop, dayWidth, dayHeight));
                    curLeft += dayWidth + 1;
                    ...
                    {
                        _dayNameHeaderColumns[i] = new Rect(curLeft, 0, dayWidth, DayNameHeadersHeight);
                    }

                    if (Calendar.Days[i].Date.DayOfWeek == Calendar.FirstDayOfWeek)
                    {
                        Calendar.Weeks[j++].SetBounds(new Rect(0, curTop, Calendar.ClientSize.Width, dayHeight));
                    }
                } 
                #endregion
            }
            ...
            if (Calendar.DaysMode == CalendarDaysMode.Expanded)
            {
                ...
                foreach (CalendarItem item in Calendar.Items)
                {
                    ...
                    if (item.IsOnDayTop)
                    {
                        ...
                        item.SetBounds(Rect.FromLTRB(dayStart.DayTop.Bounds.Left, 0, dayEnd.DayTop.Bounds.Right, 1));
                ...
                foreach (CalendarDay day in Calendar.Days)
                {
                    ...
                    foreach (List<CalendarItem> group in groups)
                    {
                        ...
                        foreach (CalendarItem item in group)
                        {
                            ...
                            item.SetBounds(Rect.FromLTRB(rleft, rtop, right, bottom));
            ...
            else if (Calendar.DaysMode == CalendarDaysMode.Short)
            {
                ...
                for (int week = 0; week < Calendar.Weeks.Length; week++)
                {
                    ...
                    foreach (CalendarItem item in Calendar.Items)
                    {
                        ...
                        Rect rStart = dayStart.Bounds;
                        Rect rEnd = dayEnd.Bounds;
                        int rtop = rStart.Top + DayHeaderHeight + top * StandardItemHeight;
                        Rect r = Rect.FromLTRB(rStart.Left, rtop, rEnd.Right, rtop + StandardItemHeight);
        ...
        {
            TextRenderer.DrawText(e.DrawingContext, e.Text, e.Font, e.Bounds, e.TextColor, e.Format);
        }
        ...
        public virtual void OnDrawTimeScale(CalendarRendererEventArgs e)
        {
            ...
            for (int i = 0; i < e.Calendar.Days[0].TimeUnits.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(minutes))
                {
                    ...
                    CalendarRendererBoxEventArgs hevt = new CalendarRendererBoxEventArgs(e, new Rect(hourLeft, unit.Bounds.Top, hourWidth, unit.Bounds.Height), hours, TextFormatFlags.Right);
                    ...
                    CalendarRendererBoxEventArgs mevt = new CalendarRendererBoxEventArgs(e, new Rect(minuteLeft, unit.Bounds.Top, minuteWidth, unit.Bounds.Height), minutes, TextFormatFlags.Top | TextFormatFlags.Left);
        ...
        {
            Rect days = e.Calendar.DaysBodyRect; days.Inflate(-1, -1);
            Region oldclip = e.DrawingContext.Clip;
            ...
            foreach (CalendarItem item in e.Calendar.Items)
            {
                ...
                {
                    e.DrawingContext.SetClip(days, CombineMode.Intersect);
                    clipped = true;
                }

                List<Rect> rects = new List<Rect>(item.GetAllBounds());
                ...
                if (clipped)
                    e.DrawingContext.SetClip(oldclip, CombineMode.Replace);
            } 
            #endregion
            ...
            foreach (CalendarItem item in e.Calendar.Items)
            {
                ...
                for (int i = 0; i < rects.Count; i++)
                {
                    ...
                    SmoothingMode smbuff = e.DrawingContext.SmoothingMode;
                    e.DrawingContext.SmoothingMode = SmoothingMode.HighQuality;

                    OnDrawItemBorder(evt);

                    e.DrawingContext.SmoothingMode = smbuff;
                }
        ...
        {
            List<Rect> rects = new List<Rect>(e.Item.GetAllBounds());
            ...
            for (int i = 0; i < rects.Count; i++)
            {
                ...
                if (!evt.Item.PatternColors.IsEmpty)
                {
        ...
        {
            foreach (Rect bounds in e.Item.GetAllBounds())
            {
        ...
        public virtual void OnDrawItemContent(CalendarRendererItemBoundsEventArgs e)
        {
            ...
            {
                Rect bounds = Rects[i];
                Rect imageBounds = Rect.Empty;
                Rect rStartTime = new Rect();
                Rect rEndTime = new Rect();
                string endTime = string.Empty;
                string startTime = string.Empty;
                Color secondaryForecolor = e.Item.ForeColor;

                if (e.Item.ShowEndTime && i == Rects.Count - 1)
                {
                    endTime = e.Item.EndDateText;
                    rEndTime = new Rect(Point.Empty, TextRenderer.MeasureText(endTime, e.Calendar.Font));
                    rEndTime = Rect.FromLTRB(bounds.Right - rEndTime.Width - ItemTextMargin.Right,
                ...
                if (e.Item.ShowStartTime && i == 0)
                {
                    ...
                    rStartTime = new Rect(Point.Empty, TextRenderer.MeasureText(startTime, e.Calendar.Font));
        ...
        public virtual void OnDrawItemImage(CalendarRendererItemBoundsEventArgs e)
        {
            if (e.Item.Image != null)
            {
                e.DrawingContext.DrawImage(e.Item.Image, e.Bounds);

                //e.DrawingContext.FillRect(Brushes.Red, e.Bounds);
            }
        }
        ...
        {
            //e.DrawingContext.FillRect(Brushes.Red, e.Day.OverflowEndBounds);
        }
        ...
        public virtual void OnDrawWeekHeaders(CalendarRendererEventArgs e)
        {
            ...
            for (int i = 0; i < Calendar.Weeks.Length; i++)
            {
                ...
                SizeF sz = e.DrawingContext.MeasureString(str, e.Calendar.Font);
        ...
        public virtual void OnDrawWeekHeader(CalendarRendererBoxEventArgs e)
        {
            ...
            sf.LineAlignment = HorizontalAlignment.Center;
            sf.Alignment = HorizontalAlignment.Center;

            using (SolidBrush b = new SolidBrush(e.TextColor))
            {
                e.DrawingContext.DrawString(e.Text, e.Font, b, e.Bounds, sf);
            }

            e.DrawingContext.ResetTransform();
