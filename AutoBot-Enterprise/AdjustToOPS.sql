...
SELECT       coalesce([#StockDifferences].InvoiceNo, [#OPS to Zero - Overs].InvoiceNo, [#P2O-Returns].InvoiceNo) as InvoiceNo,
			 ...
			 (isnull(avg([#StockDifferences].[OPS-Quantity]),0) +  avg(isnull(Adj.Quantity,0)) + avg(isnull([#P2O-Returns].Quantity,0)) + avg(isnull([#OPS to Zero - Overs].Quantity,0)) - isnull(sum([Asycuda-Quantity]), 0) ) AS Diff,
...
declare @ItemNumber varchar(50) = 'AMZ/102716N'
select 'Adjustments Data'
select * from  #adjmentsData where itemnumber = @ItemNumber
select 'Sales Data'
select * from  #salesData where itemnumber = @ItemNumber
