...
#endif
{
    ...
    public static class DbContextExtensions
    {
           ...
           // if (!Exists(context, item)) return;
                    ...
                    && item.TrackingState != TrackingState.Deleted))
            {
                ...
                else
                {
                    ...
                    {
                       if(item.TrackingState != TrackingState.Unchanged) throw;
                    }
        ...
        public static bool Exists(DbContext context,object TEntity) 
        {
            string entityName = TEntity.GetType().Name;
            ...
            CancellationToken cancellationToken)
        {
            ...
            // Get related entities
            List<object> entities = await context.ExecuteQueryEntitySqlAsync(entitySql, cancellationToken).ConfigureAwait(false);
            return entities;
        } 
#endif
            ...
            CancellationToken cancellationToken)
        {
            var objContext = ((IObjectContextAdapter)dbContext).ObjectContext;
            var objQuery = new ObjectQuery<object>(entitySql, objContext);
            ObjectResult<object> result = await objQuery.ExecuteAsync(MergeOption.NoTracking, cancellationToken).ConfigureAwait(false);
            return result.ToList();
        }
#endif
