    ...
    public class FileUtils
    {
            ...
            string[] poContacts)
        {
            ...
            using (var ctx = new CoreEntitiesContext())
            {
                try
                {
                    ...
                    var poInfo = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(docSet.AsycudaDocumentSetId));
            ...
            //(int? fileTypeId, int? emailId, bool overWriteExisting, List<AsycudaDocumentSet> docSet, string fileType)
        {
            ...
            foreach (var file in csvFiles.Where(x => x.Extension.ToLower() == ".pdf"))
            {
                ...
                var success = InvoiceReader.Import(file.FullName, fileTypeId.GetValueOrDefault(), emailId, true, SaveCSVModel.Instance.GetDocSets(fileType), fileType, FileUtils.Client);
            ...
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    foreach (var data in lst)
                    {
                        ...
                        var info = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(emailIds.First().AsycudaDocumentSetId));
        ...
        private static void SubmitDocSetDiscrepanciesPreAssessmentReportToCustoms(FileTypes fileType)
        {
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    var info = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(fileType.AsycudaDocumentSetId));
                    var directory = info.Item2;
        ...
        private static void SubmitSalesToCustoms(IEnumerable<IGrouping<string, TODO_SubmitDiscrepanciesToCustoms>> lst)
        {
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    var info = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(RES.First().AsycudaDocumentSetId));
                    var directory = info.Item2;
        ...
        private static void SubmitMissingInvoices(FileTypes ft)
        {
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    foreach (var emailIds in lst)
                    {
                        if (Enumerable.Any<ActionDocSetLogs>(GetDocSetActions(emailIds.Key.AsycudaDocumentSetId, "SubmitMissingInvoices"))) continue;
        ...
        private static void SubmitMissingInvoicePDFs(FileTypes ft)
        {
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    foreach (var emailIds in lst)
                    {
                        if (Enumerable.Any<ActionDocSetLogs>(GetDocSetActions(emailIds.Key.AsycudaDocumentSetId, "SubmitMissingInvoicePDFs"))) continue;
            ...
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    var lst = ctx.TODO_SubmitEntryCIF
                        ...
                        .ToList();
                    if (!lst.Any()) return;
                    if (Enumerable.Any<ActionDocSetLogs>(GetDocSetActions(ft.AsycudaDocumentSetId, "SubmitEntryCIF"))) return;
                    ...
                    var body = $"Please see attached CIF for Shipment: {lst.First().Declarant_Reference_Number} . \r\n" +
                               ...
                               $"AutoBot";
                    List<string> attlst = new List<string>();
                    var poInfo = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(ft.AsycudaDocumentSetId));
            ...
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    foreach (var email in emails)
                    {
                        ...
                        // if (GetDocSetActions(email.Key.AsycudaDocumentSetId, "SubmitUnclassifiedItems").Any()) continue;
                        var info = Enumerable.FirstOrDefault<Tuple<AsycudaDocumentSetEx, string>>(CurrentPOInfo(email.Key.AsycudaDocumentSetId));
                        var directory = info.Item2;
        ...
        public static void SubmitIncompleteSuppliers(FileTypes ft)
        {
            ...
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    if (Enumerable.Any<ActionDocSetLogs>(GetDocSetActions(ft.AsycudaDocumentSetId, "SubmitIncompleteSuppliers"))) return;
            ...
            try
            {
                ...
                using (var ctx = new CoreEntitiesContext())
                {
                    ...
                    foreach (var docSet in lst)
                    {
                        ...
                        if (Enumerable.Any<ActionDocSetLogs>(GetDocSetActions(docSet.AsycudaDocumentSetId, "SubmitInadequatePackages"))) continue;
            ...
            using (var ctx = new CoreEntitiesContext())
            {
                ...
                if (File.Exists(errorfile))
                {
                    var body = "The following entries are expiring within the next month. \r\n" +
                               ...
                               $"Regards,\r\n" +
                               $"AutoBot";
                    EmailDownloader.EmailDownloader.SendEmail(Client, directory, $"Entries Expiring {DateTime.Now:yyyy-MM-dd} - {DateTime.Now.AddMonths(1):yyyy-MM-dd}", contacts.Select(x => x.EmailAddress).Distinct().ToArray(), body, new string[] { errorfile });
                }
            }
                        ...
                        new ExportToCSV<TODO_PODocSetToAssessErrors, List<TODO_PODocSetToAssessErrors>>()
 {
     dataToPrint = res
        ...
        public static void ClearDocSetEntries(FileTypes fileType)
        {
            ...
            Utils.ClearDocSetEntryData(fileType.AsycudaDocumentSetId);
            Utils.ClearDocSetAttachments(fileType.AsycudaDocumentSetId, fileType.EmailId);
        ...
        private static void CreatePOEntries(int docSetId, List<int> entrylst)
        {
            try
            {
                ...
                var po = FileUtils.CurrentPOInfo(docSetId).FirstOrDefault();
                        ...
                        foreach (var doc in doclst)
                        {
                            try
                            {
                                ...
                                if (data != null)
                                {
                                    ...
                                    s.dataToPrint = Enumerable.ToList<Utils.SaleReportLine>(data);
                                    s.SaveReport(path);
                                }
                                else
                                {
        ...
        public static async Task<IEnumerable<Utils.SaleReportLine>> GetDocumentSalesReport(int ASYCUDA_Id)
        {
            ...
            if (Enumerable.Any<Utils.SaleReportLine>(alst)) return alst;
                ...
                catch (Exception e)
                {
                    ...
                    using (var ctx = new CoreEntitiesContext())
                    {
                                ...
                                && x.Status == "Sender Informed of Error") == null)
                        {
                            ...
                            EmailDownloader.EmailDownloader.SendBackMsg(emailId, FileUtils.Client, body);
                            ctx.AttachmentLog.Add(new AttachmentLog(true)
                            {
        ...
        public static void ExecuteDataSpecificFileActions(FileTypes fileType, FileInfo[] files, ApplicationSettings appSetting)
        {
            ...
            {
                EmailDownloader.EmailDownloader.ForwardMsg(fileType.EmailId,FileUtils.Client, $"Bug Found",
