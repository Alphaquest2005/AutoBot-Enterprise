    ...
    public class SessionsUtils
    {
        public static Dictionary<string, Func<ILogger, Task>> SessionActions =>
            new Dictionary<string, Func<ILogger, Task>>(WaterNut.DataSpace.Utils.ignoreCase)
            {
                ...
                {"AllocateSales", (log) => new AllocateSalesUtils(log).AllocateSales() },
                ...
                {"ImportPOEntries", (log) => new DocumentUtils(log).ImportPOEntries(false) },
                ...
                {"EmailWarehouseErrors", (log) => EX9Utils.EmailWarehouseErrors(log) },
                ...
                {"SubmitMissingInvoices", (log) => Utils.SubmitMissingInvoices(new FileTypes(), log) },
                {"SubmitIncompleteEntryData",(log) => Utils.SubmitIncompleteEntryData(new FileTypes(), log) },
                ...
                // {"ImportPDF", (log) => PDFUtils.ImportPDF(null, new FileTypes(), log) }, // Commented out - requires FileInfo[] parameter, not null
