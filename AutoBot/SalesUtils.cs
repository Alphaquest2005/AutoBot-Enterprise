...
namespace AutoBot
{
    public class SalesUtils
    {
        ...
        {
            var doclst = await GetSalesDocumentsWithEntryData
        ...
        {
            var docset = await BaseDataModel.Instance.GetDocSetWithEntryDataDocs(asycudaDocumentSetId)
            ...
            using (var ctx = new CoreEntitiesContext())
            {
                ...
                if (File.Exists(errorfile))
                    EmailDownloader.EmailDownloader.SendEmail(Utils.Client, directory,
                        ...
                        contacts.Select(x => x.EmailAddress).ToArray(), "Please see attached...", new[]
                        {
                            errorfile
                        });
            }
        ...
        public static void AllocateSales()
        {
            ...
            using (var ctx = new CoreEntitiesContext())
            {
                if (BaseDataModel.Instance.CurrentApplicationSettings.AssessEX == true
                    && !ctx.TODO_UnallocatedSales.Any(x =>
                        x.ApplicationSettingsId ==
                    ...
                    foreach (var emailIds in lst)
                    {
                        ...
                        if (emailIds.Key == null)
                            ...
                            EmailDownloader.EmailDownloader.ForwardMsg(emailIds.Key, Utils.Client,
                                "Assessed Ex-Warehouse Entries", body, contacts, attlst.ToArray());
